...
// 4A1: issue-only phase-1 gemm (commit, NO wait0) — caller waits at the next loop top so the
//   committed QK/dOV group stays in flight across the epilogue, keeping the tensor pipe fed.
...
// 4B: reduce the fp32 dQ fragment register->GLOBAL directly (atomicAdd = red.global.add.f32),
//   skipping the smem stage + TMA-reduce. Moves the dQ scatter off the SATURATED shared/LSU pipe
//   (65% busy, 27.6M store bank conflicts) onto the IDLE global path (DRAM ~15%). Frees sS/sdP (~64KB).
//   Fragment map == store_acc_sw128_f32: thread(w,lane) owns rows {r0,r1}, cols nt*8+cc + {0,1}.
__device__ __forceinline__ void red_acc_global_f32(
    const float* dq, float* __restrict__ d_dq_accum, long crowBase, int wg, int wtid, float scl, int D) {
    const int w = wtid >> 5, lane = wtid & 31;
    const int r0 = w * 16 + (lane >> 2), r1 = r0 + 8, cc = (lane & 3) * 2;
    const long base0 = (crowBase + r0) * (long)D + (long)(wg * 64);
    const long base1 = (crowBase + r1) * (long)D + (long)(wg * 64);
#pragma unroll
    for (int nt = 0; nt < 8; nt++) {
        const int c = nt * 8 + cc;
        atomicAdd(&d_dq_accum[base0 + c    ], dq[nt*4+0] * scl);
        atomicAdd(&d_dq_accum[base0 + c + 1], dq[nt*4+1] * scl);
        atomicAdd(&d_dq_accum[base1 + c    ], dq[nt*4+2] * scl);
        atomicAdd(&d_dq_accum[base1 + c + 1], dq[nt*4+3] * scl);
    }
}
template<int Br, int Bc, int D>
__global__ void __launch_bounds__(384, 1)
...
gqa_backward_v4b_kv(
    ...
    const __grid_constant__ CUtensorMap tma_dV_st,
    const __grid_constant__ CUtensorMap tma_dK_st,
    float * __restrict__ d_dq_accum,   // 4B: raw dq_accum for register->global atomic reduce
    const float * __restrict__ d_Drow,
    const float * __restrict__ d_LSE,
    bf16 * __restrict__ d_dK, bf16 * __restrict__ d_dV,
    int B, int Hq, int Hkv, int G, int S, float scale
    ...
    for (int it = 0; it < nIter; it++) {
        ...
        run_gemm_dKdQ_te_issue_ho(dk, dq, descDSmn, descDSk, descKhalf, sQ_sw[s] + wg * 4096);
        run_gemm_dVdKdQ_te_wait(dv, dk, dq);
        // 4B: dQ register->global atomic reduce (off saturated LSU, onto idle DRAM); no smem stage / TMA-reduce
        red_acc_global_f32(dq, d_dq_accum, lBaseOf(gC, qcC), wg, wtid, scale, D);
        consumer_sync();
        if (tid == 0) mbar_arrive_v11(&empty[s]);
        if (++qcC == nQTiles) { qcC = qc0; ++gC; }
    }
...
void launch_gqa_backward_v4b(
    ...
    auto make_tma_red = [&](const float* ptr, uint64_t rows) {
        ...
        CUresult r=cuTensorMapEncodeTiled(&desc,CU_TENSOR_MAP_DATA_TYPE_FLOAT32,2,(void*)ptr,gSize,gStride,box,eStride,
            CU_TENSOR_MAP_INTERLEAVE_NONE,CU_TENSOR_MAP_SWIZZLE_128B,CU_TENSOR_MAP_L2_PROMOTION_L2_256B,CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE);
        if(r!=CUDA_SUCCESS){const char*e;cuGetErrorString(r,&e);fprintf(stderr,"tma_red v4b: %s\n",e);exit(1);} return desc; };
    ...
    gqa_backward_v4b_kv<Br,Bc,D><<<GRID, BLOCK>>>(
        tma_K_sw, tma_V_sw, tma_Q_sw, tma_dO_sw, tma_dV_st, tma_dK_st, d_dq_accum,
        d_Drow, d_LSE, d_dK, d_dV, B, Hq, Hkv, G, S, scale);
...
gqa_backward_v4a1_kv(
    ...
    static_assert(Br == 64 && Bc == 64 && D == 128, "V4A1 requires Br=Bc=64, D=128");
    constexpr int CONS = 256;
    constexpr int PD   = 3;
    ...
    __shared__ __align__(128)  bf16 sQ_sw [PD][Br * D];
    __shared__ __align__(128)  bf16 sdO_sw[PD][Br * D];
    __shared__ __align__(1024) float sS [2][Br * 64];   // V4A1: DOUBLE-BUFFERED SWIZZLED dQ stage (wg0); each buf = 2 SW128B atoms of [Br*32]
    __shared__ __align__(1024) float sdP[2][Br * 64];   // V4A1: DOUBLE-BUFFERED SWIZZLED dQ stage (wg1); atom a at [a*Br*32], SW128B chunk^ (row&7)
    __shared__ __align__(1024) bf16  sP [Br * 64];
    __shared__ __align__(1024) bf16  sDS[Br * 64];
    __shared__                 float sLSE[PD][Br];
    __shared__                 float sD  [PD][Br];
    ...
    // 4A1: MMA software-pipeline. Peel phase-1 (QK for wg0 / dOV for wg1) into a prologue, and issue
    //   tile it+1's phase-1 during tile it's epilogue so the async wgmma group runs through the
    //   store/reduce/barriers -> tensor pipe stays fed (was 38% -> aim higher). pacc is reused (free
    //   after fused_p_stsm / fuse_dS) so this is register/smem-neutral vs V44.
    float pacc[32]; zeroN<32>(pacc);
    {   const int s0 = 0;
        mbar_wait_v4(&full[s0], cpar[s0]); cpar[s0] ^= 1;
        mbar_wait_v4(&d_ready[s0], dpar[s0]); dpar[s0] ^= 1;
        if (wg == 0) run_gemm_n64_sw2_hoB_issue(pacc, sQ_sw[s0],  descGemmB);
        else         run_gemm_n64_sw2_hoB_issue(pacc, sdO_sw[s0], descGemmB);
    }
    ...
    for (int it = 0; it < nIter; it++) {
        ...
        wgmma_wait0();                              // drain tile it's phase-1 (issued last iter / prologue)
        fence_operandN<32>(pacc);
        if (wg == 0) {
            if (qcC == qc0) fused_p_stsm<Bc, true >(pacc, sP, sLSE[s], wtid, q_row0, k_row0, scale2);
            else            fused_p_stsm<Bc, false>(pacc, sP, sLSE[s], wtid, 0,       0,      scale2);
        }
        consumer_sync();                            // barrier1: sP visible

        run_gemm_dVdK_half_te_issue_hoA(dv, descP, sdO_sw[s] + wg * 4096);   // wgmma group A

        if (wg == 1) fuse_dS_ldstsm<Bc>(sP, pacc, sD[s], sDS, wtid);         // consume pacc(dP) before reuse
        consumer_sync();                            // barrier2: sDS visible

        float dq[32]; zeroN<32>(dq);
        run_gemm_dKdQ_te_issue_ho(dk, dq, descDSmn, descDSk, descKhalf, sQ_sw[s] + wg * 4096);  // group B

        if (has_next) {                             // 4A1: issue tile it+1 phase-1 -> group C (runs thru epilogue)
            const int s2 = (it + 1) % PD;
            mbar_wait_v4(&full[s2], cpar[s2]); cpar[s2] ^= 1;
            mbar_wait_v4(&d_ready[s2], dpar[s2]); dpar[s2] ^= 1;
            zeroN<32>(pacc);
            if (wg == 0) run_gemm_n64_sw2_hoB_issue(pacc, sQ_sw[s2],  descGemmB);
            else         run_gemm_n64_sw2_hoB_issue(pacc, sdO_sw[s2], descGemmB);
        }

        if (has_next) wgmma_wait1();                // drain A(dVdK)+B(dKdQ); keep C(it+1) in flight
        else          wgmma_wait0();                // last tile: no C -> drain A+B
        fence_operandN<32>(dv); fence_operandN<32>(dk); fence_operandN<32>(dq);
...
void launch_gqa_backward_v4a1(
    ...
    auto make_tma_red = [&](const float* ptr, uint64_t rows) {
        ...
        CUresult r=cuTensorMapEncodeTiled(&desc,CU_TENSOR_MAP_DATA_TYPE_FLOAT32,2,(void*)ptr,gSize,gStride,box,eStride,
            CU_TENSOR_MAP_INTERLEAVE_NONE,CU_TENSOR_MAP_SWIZZLE_128B,CU_TENSOR_MAP_L2_PROMOTION_L2_256B,CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE);
        if(r!=CUDA_SUCCESS){const char*e;cuGetErrorString(r,&e);fprintf(stderr,"tma_red v4a1: %s\n",e);exit(1);} return desc; };
    ...
    constexpr dim3 BLOCK(384);
    dim3 GRID(B, Hkv, S / Bc);
    gqa_backward_v4a1_kv<Br,Bc,D><<<GRID, BLOCK>>>(
        tma_K_sw, tma_V_sw, tma_Q_sw, tma_dO_sw, tma_dV_st, tma_dK_st, tma_dq_red,
        d_Drow, d_LSE, d_dK, d_dV, B, Hq, Hkv, G, S, scale);
...
gqa_backward_v4a2_kv(
    ...
    static_assert(Br == 64 && Bc == 64 && D == 128, "V4A2 requires Br=Bc=64, D=128");
    constexpr int CONS = 256;
    constexpr int PD   = 3;
    ...
    __shared__ __align__(128)  bf16 sQ_sw [PD][Br * D];
    __shared__ __align__(128)  bf16 sdO_sw[PD][Br * D];
    __shared__ __align__(1024) float sS [2][Br * 64];   // V4A2: DOUBLE-BUFFERED SWIZZLED dQ stage (wg0); each buf = 2 SW128B atoms of [Br*32]
    __shared__ __align__(1024) float sdP[2][Br * 64];   // V4A2: DOUBLE-BUFFERED SWIZZLED dQ stage (wg1); atom a at [a*Br*32], SW128B chunk^ (row&7)
    __shared__ __align__(1024) bf16  sP [Br * 64];
    __shared__ __align__(1024) bf16  sDS[Br * 64];
    __shared__                 float sLSE[PD][Br];
    __shared__                 float sD  [PD][Br];
    ...
    // 4A2: MMA software-pipeline. Peel phase-1 (QK for wg0 / dOV for wg1) into a prologue, and issue
    //   tile it+1's phase-1 during tile it's epilogue so the async wgmma group runs through the
    //   store/reduce/barriers -> tensor pipe stays fed (was 38% -> aim higher). pacc is reused (free
    //   after fused_p_stsm / fuse_dS) so this is register/smem-neutral vs V44.
    float pacc[32]; zeroN<32>(pacc);
    ...
    for (int it = 0; it < nIter; it++) {
        ...
        if (has_next) {                             // 4A2: issue tile it+1 phase-1 -> group C (runs thru epilogue)
            const int s2 = (it + 1) % PD;
            mbar_wait_v4(&full[s2], cpar[s2]); cpar[s2] ^= 1;
            mbar_wait_v4(&d_ready[s2], dpar[s2]); dpar[s2] ^= 1;
            zeroN<32>(pacc);
        ...
        fence_operandN<32>(dv); fence_operandN<32>(dk); fence_operandN<32>(dq);
        if (tid == 0) mbar_arrive_v11(&empty[s]);   // 4A2: free stage EARLY (Q/dO/LSE/D all consumed by wait1) -> +producer runway
...
void launch_gqa_backward_v4a2(
    ...
    auto make_tma_red = [&](const float* ptr, uint64_t rows) {
        ...
        CUresult r=cuTensorMapEncodeTiled(&desc,CU_TENSOR_MAP_DATA_TYPE_FLOAT32,2,(void*)ptr,gSize,gStride,box,eStride,
            CU_TENSOR_MAP_INTERLEAVE_NONE,CU_TENSOR_MAP_SWIZZLE_128B,CU_TENSOR_MAP_L2_PROMOTION_L2_256B,CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE);
        if(r!=CUDA_SUCCESS){const char*e;cuGetErrorString(r,&e);fprintf(stderr,"tma_red v4a2: %s\n",e);exit(1);} return desc; };
    ...
    constexpr dim3 BLOCK(384);
    dim3 GRID(B, Hkv, S / Bc);
    gqa_backward_v4a2_kv<Br,Bc,D><<<GRID, BLOCK>>>(
        tma_K_sw, tma_V_sw, tma_Q_sw, tma_dO_sw, tma_dV_st, tma_dK_st, tma_dq_red,
        d_Drow, d_LSE, d_dK, d_dV, B, Hq, Hkv, G, S, scale);
...
int main(){
    ...
    launch_gqa_backward_v4a1<Br2,Bc2,D>(d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale);
    CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaDeviceSynchronize());
    check("── 4A1 Br=64 Bc=64 MMA-pipeline (phase-1 prefetch) (Hopper SM_90) ──", Nq, Nkv, d_dQ, d_dK, d_dV);

    launch_gqa_backward_v4a2<Br2,Bc2,D>(d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale);
    CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaDeviceSynchronize());
    check("── 4A2 Br=64 Bc=64 prefetch + early stage-free (Hopper SM_90) ──", Nq, Nkv, d_dQ, d_dK, d_dV);

    launch_gqa_backward_v4b<Br2,Bc2,D>(d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale);
    CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaDeviceSynchronize());
    check("── 4B Br=64 Bc=64 dQ register->global atomic reduce (Hopper SM_90) ──", Nq, Nkv, d_dQ, d_dK, d_dV);
    ...
    {
        KernelStats s = benchmarkKernel(
            [&](){ launch_gqa_backward_v4a1<Br2,Bc2,D>(
                d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale); },
            100, 10, bwd_flops);
        displayStats("GQA bwd 4A1 Br=64, Bc=64  MMA-pipeline (phase-1 prefetch)  (Hopper SM_90)", s);
    }
    {
        KernelStats s = benchmarkKernel(
            [&](){ launch_gqa_backward_v4a2<Br2,Bc2,D>(
                d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale); },
            100, 10, bwd_flops);
        displayStats("GQA bwd 4A2 Br=64, Bc=64  prefetch + early stage-free  (Hopper SM_90)", s);
    }
    {
        KernelStats s = benchmarkKernel(
            [&](){ launch_gqa_backward_v4b<Br2,Bc2,D>(
                d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale); },
            100, 10, bwd_flops);
        displayStats("GQA bwd 4B Br=64, Bc=64  dQ register->global atomic reduce  (Hopper SM_90)", s);
    }
