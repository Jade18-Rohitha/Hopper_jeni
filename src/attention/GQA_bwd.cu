...
__device__ __forceinline__ void stage_acc_bf16_sw(const float *d, bf16 *stage, int tid, float scl) {
    int w = tid >> 5, lane = tid & 31;
    int r0 = w * 16 + (lane >> 2), r1 = r0 + 8, cc = (lane & 3) * 2;
...
#pragma unroll
    for (int nt = 0; nt < 8; nt++) {
        ...
        stage[r0 * 64 + ((chunk ^ ph0) << 3) + lo    ] = __float2bfloat16(d[nt*4+0]*scl);
        stage[r0 * 64 + ((chunk ^ ph0) << 3) + lo + 1] = __float2bfloat16(d[nt*4+1]*scl);
        stage[r1 * 64 + ((chunk ^ ph1) << 3) + lo    ] = __float2bfloat16(d[nt*4+2]*scl);
        stage[r1 * 64 + ((chunk ^ ph1) << 3) + lo + 1] = __float2bfloat16(d[nt*4+3]*scl);
    }
}

// V46: vectorized (bf16x2 / STS.32) swizzled dV/dK stage — the two per-iter elements
//   (lo, lo+1) are contiguous in the same 128B-swizzle chunk, so one 32-bit store
//   replaces two 16-bit stores. Identical bytes to stage_acc_bf16_sw; half the store insts.
...
gqa_backward_v45_kv(
    ...
    bf16 *stage_dk = qflat + 8192 + wg * 4096;
    fence_operandN<32>(dv);
    stage_acc_bf16_sw(dv, stage_dv, wtid, 1.0f);
    fence_operandN<32>(dk);
    stage_acc_bf16_sw(dk, stage_dk, wtid, scale);
    consumer_sync();
    fence_proxy_async_shared();
    if (wtid == 0) {
        tma_store_2d_v34(&tma_dV_st, stage_dv, (uint32_t)(wg * 64), kvFlatRow);
...
int main(){
    ...
    CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaDeviceSynchronize());
    check("── V45 Br=64 Bc=64 swizzled dV/dK stage (Hopper SM_90) ──", Nq, Nkv, d_dQ, d_dK, d_dV);

    launch_gqa_backward_v46<Br2,Bc2,D>(d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale);
    CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaDeviceSynchronize());
    check("── V46 Br=64 Bc=64 vectorized swizzled dV/dK stage (Hopper SM_90) ──", Nq, Nkv, d_dQ, d_dK, d_dV);

    launch_gqa_backward_v47<Br2,Bc2,D>(d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale);
    CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaDeviceSynchronize());
    check("── V47 Br=64 Bc=64 L2-aware grid raster (k_tile fastest) (Hopper SM_90) ──", Nq, Nkv, d_dQ, d_dK, d_dV);

    launch_gqa_backward_v48<Br2,Bc2,D>(d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale);
    CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaDeviceSynchronize());
    check("── V48 Br=64 Bc=64 vec-store + L2 raster (Hopper SM_90) ──", Nq, Nkv, d_dQ, d_dK, d_dV);
    ...
    }
    {
        KernelStats s = benchmarkKernel(
            [&](){ launch_gqa_backward_v45<Br2,Bc2,D>(
                d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale); },
            100, 10, bwd_flops);
        displayStats("GQA bwd V45 Br=64, Bc=64  swizzled dV/dK stage  (Hopper SM_90)", s);
    }
    {
        KernelStats s = benchmarkKernel(
            [&](){ launch_gqa_backward_v46<Br2,Bc2,D>(
                d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale); },
            100, 10, bwd_flops);
        displayStats("GQA bwd V46 Br=64, Bc=64  vectorized swizzled dV/dK stage  (Hopper SM_90)", s);
    }
    {
        KernelStats s = benchmarkKernel(
            [&](){ launch_gqa_backward_v47<Br2,Bc2,D>(
                d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale); },
            100, 10, bwd_flops);
        displayStats("GQA bwd V47 Br=64, Bc=64  L2-aware grid raster (k_tile fastest)  (Hopper SM_90)", s);
    }
    {
        KernelStats s = benchmarkKernel(
            [&](){ launch_gqa_backward_v48<Br2,Bc2,D>(
                d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale); },
            100, 10, bwd_flops);
        displayStats("GQA bwd V48 Br=64, Bc=64  vec-store + L2 raster  (Hopper SM_90)", s);
    }
