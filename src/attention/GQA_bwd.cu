...
// ═════════════════════════════════════════════════════════════════════════════
// V45 = V44 with the dQ-stage swizzle changed 128B -> 64B (probe candidate for the floored 5.3-way
// store conflict).  SWIZZLE_64B fp32 = 16 fp32/row (64B atom) = 4 chunks of 4 fp32, swizzle chunk^(row&3),
// period 4 rows.  The 64-wide D-half is stored as FOUR 64x16 swizzled atoms (buf[a] at +a*Br*16) and
// reduced with FOUR cp.reduce calls per wg (vs V44's TWO 64x32 128B atoms).  Everything else = V44.
//
// Bit-identical: pure address permutation.  fragment col c -> global col wg*64+c is the SAME identity as
// V44 (atom=c>>4, c16=c&15 => wg*64 + (c>>4)*16 + (c&15) = wg*64 + c).  Same fp32 values, same dq_accum,
// same add-order class -> dQ max_abs bound holds (H200-gated check validates the 64B swizzle match).
//                                                            [SM_90a only]
// ── Swizzled fp32 store: dQ D-fragment -> 64B-swizzled smem (4 atoms of 64x16).  Same fragment mapping
//    (r0/r1 rows, c cols) as store_acc_sw128_f32 but routes to the SW64B slot: atom=c>>4, col16=c&15,
//    chunk=col16>>2, phys = atom*(64*16) + row*16 + ((chunk ^ (row&3))<<2) + (col16&3).
...
#pragma unroll
    for (int nt = 0; nt < 8; nt++) {
        const int c   = nt * 8 + cc;               // 0..63 (even)
        const int atom = c >> 4, col16 = c & 15;   // 64B atom (0..3), col within atom
        const int chunk = col16 >> 2, lo = col16 & 3;
        const int abase = atom * (64 * 16);        // atom base = a*1024 fp32
        base[abase + r0 * 16 + ((chunk ^ ph0) << 2) + lo    ] = d[nt * 4 + 0] * scl;
        base[abase + r0 * 16 + ((chunk ^ ph0) << 2) + lo + 1] = d[nt * 4 + 1] * scl;
        base[abase + r1 * 16 + ((chunk ^ ph1) << 2) + lo    ] = d[nt * 4 + 2] * scl;
        base[abase + r1 * 16 + ((chunk ^ ph1) << 2) + lo + 1] = d[nt * 4 + 3] * scl;
    }
}
template<int Br, int Bc, int D>
__global__ void __launch_bounds__(384, 1)
gqa_backward_v45_kv(
    ...
    const __grid_constant__ CUtensorMap tma_dV_st,
    const __grid_constant__ CUtensorMap tma_dK_st,
    const __grid_constant__ CUtensorMap tma_dq_red,   // fp32 dq_accum (SWIZZLE_64B, box 16x64) — swizzled TMA-reduce add target
    const float * __restrict__ d_Drow,
    const float * __restrict__ d_LSE,
    bf16 * __restrict__ d_dK, bf16 * __restrict__ d_dV,
    int B, int Hq, int Hkv, int G, int S, float scale
    ...
    __shared__ __align__(128)  bf16 sQ_sw [PD][Br * D];
    __shared__ __align__(128)  bf16 sdO_sw[PD][Br * D];
    __shared__ __align__(1024) float sS [2][Br * 64];   // V45: DOUBLE-BUFFERED SWIZZLED dQ stage (wg0); each buf = 4 SW64B atoms of [Br*16]
    __shared__ __align__(1024) float sdP[2][Br * 64];   // V45: DOUBLE-BUFFERED SWIZZLED dQ stage (wg1); atom a at [a*Br*16], SW64B chunk^ (row&3)
    __shared__ __align__(1024) bf16  sP [Br * 64];
    __shared__ __align__(1024) bf16  sDS[Br * 64];
    __shared__                 float sLSE[PD][Br];
    __shared__                 float sD  [PD][Br];
    ...
    for (int it = 0; it < nIter; it++) {
        ...
        float* stageDQ = (wg == 0) ? sS[db] : sdP[db];
        store_acc_sw64_f32(dq, stageDQ, wtid, scale);                // SW64B swizzled store -> buf[it&1] (4 SW64B atoms of 64x16)
        if (wg == 0) consumer_sync_wg0(); else consumer_sync_wg1();
        fence_proxy_async_shared();                 // generic STS staging -> async-proxy (TMA) read
        if (wtid == 0) {                            // SW64B TMA-reduce: 4 atoms (cols [wg*64+a*16,+16)) -> dq_accum, off L1TEX
            const uint32_t crow = (uint32_t)lBaseOf(gC, qcC);
            tma_reduce_add_2d_v43(&tma_dq_red, stageDQ + 0 * 1024, (uint32_t)(wg * 64 +  0), crow);   // atom 0
            tma_reduce_add_2d_v43(&tma_dq_red, stageDQ + 1 * 1024, (uint32_t)(wg * 64 + 16), crow);   // atom 1
            tma_reduce_add_2d_v43(&tma_dq_red, stageDQ + 2 * 1024, (uint32_t)(wg * 64 + 32), crow);   // atom 2
            tma_reduce_add_2d_v43(&tma_dq_red, stageDQ + 3 * 1024, (uint32_t)(wg * 64 + 48), crow);   // atom 3
            tma_store_commit_v34();
            tma_bulk_wait1_v43();                   // DOUBLE-BUFFER: keep <=1 reduce pending -> reduce(it) overlaps compute(it+1);
        }
        consumer_sync();
...
void launch_gqa_backward_v45(
    ...
    auto make_tma_out = [&](const bf16* ptr, uint64_t rows) {
        ...
        CUresult r=cuTensorMapEncodeTiled(&desc,CU_TENSOR_MAP_DATA_TYPE_BFLOAT16,2,(void*)ptr,gSize,gStride,box,eStride,
            CU_TENSOR_MAP_INTERLEAVE_NONE,CU_TENSOR_MAP_SWIZZLE_NONE,CU_TENSOR_MAP_L2_PROMOTION_L2_256B,CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE);
        if(r!=CUDA_SUCCESS){const char*e;cuGetErrorString(r,&e);fprintf(stderr,"tma_out v45: %s\n",e);exit(1);} return desc; };
    CUtensorMap tma_dV_st = make_tma_out(d_dV, Rkv);
    CUtensorMap tma_dK_st = make_tma_out(d_dK, Rkv);
    // V45: fp32 dq_accum SWIZZLE_64B TMA-reduce descriptor. SW64B => box inner = 16 fp32 (=64B atom); the
    // 64-wide dQ D-half is reduced as FOUR 16-wide atoms.  Probe-confirmed: FP32 SW64B box{16,64} valid.
    auto make_tma_red = [&](const float* ptr, uint64_t rows) {
        CUtensorMap desc{};
        uint64_t gSize[2]={(uint64_t)D, rows}; uint64_t gStride[1]={(uint64_t)D*sizeof(float)};
        uint32_t box[2]={16u,64u}; uint32_t eStride[2]={1,1};
        CUresult r=cuTensorMapEncodeTiled(&desc,CU_TENSOR_MAP_DATA_TYPE_FLOAT32,2,(void*)ptr,gSize,gStride,box,eStride,
            CU_TENSOR_MAP_INTERLEAVE_NONE,CU_TENSOR_MAP_SWIZZLE_64B,CU_TENSOR_MAP_L2_PROMOTION_L2_256B,CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE);
...
int main(){
    ...
    CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaDeviceSynchronize());
    check("── V45 Br=64 Bc=64 SWIZZLE_64B dQ stage (Hopper SM_90) ──", Nq, Nkv, d_dQ, d_dK, d_dV);
    ...
    }
    {
        KernelStats s = benchmarkKernel(
            [&](){ launch_gqa_backward_v45<Br2,Bc2,D>(
                d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale); },
            100, 10, bwd_flops);
        displayStats("GQA bwd V45 Br=64, Bc=64  SWIZZLE_64B dQ stage  (Hopper SM_90)", s);
    }
