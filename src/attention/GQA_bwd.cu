...
gqa_backward_v27_kv(
    ...
    for (int it = 0; it < nIter; it++) {
        ...
        // dS = P ⊙ (dP − D) → swizzled sDS.  D loaded by wg2 → wait d_ready[s] first.
        mbar_wait_v4(&d_ready[s], dpar[s]); dpar[s] ^= 1;
        {   // V18/V19: vectorize dS — 2 adjacent columns/step (bf16×2 + float2).
            const int cbase = (2 * tid) % Bc, c8 = cbase >> 3, clo = cbase & 7;
            for (int pp = tid; pp < Br * Bc / 2; pp += CONS) {
        ...
        // dK += dSᵀ·Q  — TRANSPOSE-ELIMINATED: A = dSᵀ read DIRECTLY from swizzled sDS.
        run_gemm_dVdK_half_te(dk, sDS, sQ_sw[s] + wg * 4096);
        consumer_sync();
        if (tid == 0) mbar_arrive_v11(&empty[s]);

        // dQ_tile = dS*K (transient) - V23: TRANSPOSE-STAGING ELIMINATED. dS read
        // DIRECTLY from swizzled sDS as a Major::K A-operand (col=k=contraction);
        // the ldmatrix->stmatrix staging round-trip + its consumer_sync are DELETED
        // (sDS=dS already made cross-warp visible by the dK consumer_sync above).
        { float acc[32]; zeroN<32>(acc);
...
// ═══════════════════════════════════════════════════════════════════════════════
// V31 — SINGLE-WARPGROUP-PER-TILE (step 1). ONE consumer wg (wg0) does a FULL kv-tile
// (S+softmax+dP+dS+dV+dK+dQ, full dv[64]/dk[64]); wg1 = producer. 256 threads.
// smem-source wgmma (RS is step 2). Because ONE wg holds P (softmax) AND dP, dS is
// register/smem-local — no cross-wg flow. EXPECTED SLOWER than V30 (8 warps vs 12);
// this is the correctness foundation. Step 1b adds a paired 2nd consumer wg for occupancy.
// ═══════════════════════════════════════════════════════════════════════════════
template<int Br, int Bc, int D>
__global__ void __launch_bounds__(256, 1)
gqa_backward_v31_kv(
    ...
    static_assert(Br == 64 && Bc == 64 && D == 128, "V31 requires Br=Bc=64, D=128");
    constexpr int CONS = 128;   // ONE consumer warpgroup
    constexpr int PD   = 3;
    ...
    __shared__ __align__(128)  bf16  sA_t[2 * 64 * 72];
    __shared__ __align__(8)    uint64_t mbar_kv;
    __shared__ __align__(8)    uint64_t full   [PD];
    __shared__ __align__(8)    uint64_t empty  [PD];
    ...
    if (wg == 1) {
        ...
        uint32_t epar[PD] = {0}, fpar[PD] = {0};
        int gP = 0, qcP = qc0, gD = 0, qcD = qc0;
        auto do_dload = [&](int td) {
            const int s = td % PD;
            mbar_wait_v4(&full[s], fpar[s]); fpar[s] ^= 1;
            const long dbase = lBaseOf(gD, qcD);
            if (pl < Br) sD[s][pl] = d_Drow[dbase + pl];
            producer_sync();
            if (leader) mbar_arrive_v11(&d_ready[s]);
            if (++qcD == nQTiles) { qcD = qc0; ++gD; }
        };
    ...
    // ── CONSUMER (wg 0) — FULL kv-tile in 128 threads ──
    ...
    for (int it = 0; it < nIter; it++) {
        ...
        { float acc[32]; zeroN<32>(acc); run_gemm_n64_sw2(acc, sQ_sw[s], sK_sw);
          if (qcC == qc0) fused_p_from_acc_v29<Bc>(acc, sP, sLSE, wtid, q_row0, k_row0, scale2);
          else            fused_p_nomask_v29<Bc>(acc, sP, sLSE, wtid, scale2); }
        { float acc[32]; zeroN<32>(acc); run_gemm_n64_sw2(acc, sdO_sw[s], sV_sw);
          store_acc_smem_v6<Bc, SS_STRIDE_V24>(acc, sdP, wtid, 1.0f); }
        consumer_sync_wg0();

        mbar_wait_v4(&d_ready[s], dpar[s]); dpar[s] ^= 1;
        {   const int cbase = (4 * tid) % Bc, c8 = cbase >> 3, clo = cbase & 7;
            for (int pp = tid; pp < Br * Bc / 4; pp += CONS) {
                const int r    = (4 * pp) / Bc;
                const int pidx = r * 64 + ((c8 ^ (r & 7)) << 3) + clo;
                const int sdi  = r * SS_STRIDE_V24 + cbase;
        ...
        // dV += Pᵀ·dO  and  dK += dSᵀ·Q  (FULL width: both d-halves)
        run_gemm_dVdK_half_te(dv,      sP,  sdO_sw[s] + 0);
        run_gemm_dVdK_half_te(dv + 32, sP,  sdO_sw[s] + 4096);
        run_gemm_dVdK_half_te(dk,      sDS, sQ_sw[s]  + 0);
        run_gemm_dVdK_half_te(dk + 32, sDS, sQ_sw[s]  + 4096);
        consumer_sync_wg0();
        if (tid == 0) mbar_arrive_v11(&empty[s]);

        // dQ = dS·K  (FULL width: both d-halves) → flush
        { float acc[32]; zeroN<32>(acc); run_gemm_dQ_half_te(acc, sDS, sK_sw + 0);
          store_acc_smem_v6<64, SS_STRIDE_V24>(acc, sS, wtid, scale); }
        consumer_sync_wg0();
        atomic_flush_stage_s<Br, 64, SS_STRIDE_V24>(sS, d_dq_accum, lBaseOf(gC, qcC) * D, D, 0, wtid);
        consumer_sync_wg0();
        { float acc[32]; zeroN<32>(acc); run_gemm_dQ_half_te(acc, sDS, sK_sw + 4096);
          store_acc_smem_v6<64, SS_STRIDE_V24>(acc, sS, wtid, scale); }
        consumer_sync_wg0();
        atomic_flush_stage_s<Br, 64, SS_STRIDE_V24>(sS, d_dq_accum, lBaseOf(gC, qcC) * D, D, 64, wtid);
        consumer_sync_wg0();
        if (++qcC == nQTiles) { qcC = qc0; ++gC; }
    }

    // ── Epilogue: dv[64]/dk[64] → global (both d-halves) ──
    bf16 *st0 = sA_t, *st1 = sA_t + 64 * 72;
    fence_operandN<64>(dv);
    stage_acc_bf16_s<64, 72>(dv,      st0, wtid, 1.0f);
    stage_acc_bf16_s<64, 72>(dv + 32, st1, wtid, 1.0f);
    consumer_sync_wg0();
    store_stage_vec_s<Bc, 64, 72>(st0, d_dV, kvBase, D, 0,  wtid);
    store_stage_vec_s<Bc, 64, 72>(st1, d_dV, kvBase, D, 64, wtid);
    consumer_sync_wg0();
    fence_operandN<64>(dk);
    stage_acc_bf16_s<64, 72>(dk,      st0, wtid, scale);
    stage_acc_bf16_s<64, 72>(dk + 32, st1, wtid, scale);
    consumer_sync_wg0();
    store_stage_vec_s<Bc, 64, 72>(st0, d_dK, kvBase, D, 0,  wtid);
    store_stage_vec_s<Bc, 64, 72>(st1, d_dK, kvBase, D, 64, wtid);
}
...
void launch_gqa_backward_v31(
    ...
    auto make_tma_sw128 = [&](const bf16* ptr, uint64_t total_rows, uint32_t tile_rows) {
        ...
        if(r!=CUDA_SUCCESS){const char*e;cuGetErrorString(r,&e);fprintf(stderr,"tma v31: %s\n",e);exit(1);} return desc; };
    const uint64_t Rq=(uint64_t)B*Hq*S, Rkv=(uint64_t)B*Hkv*S;
    CUtensorMap tma_K_sw=make_tma_sw128(d_K,Rkv,Bc), tma_V_sw=make_tma_sw128(d_V,Rkv,Bc);
    CUtensorMap tma_Q_sw=make_tma_sw128(d_Q,Rq,Br),  tma_dO_sw=make_tma_sw128(d_dO,Rq,Br);
    const long drowN=(long)B*Hq*S; static float* d_Drow=nullptr; static long drow_cap=0;
    if(drowN>drow_cap){ if(d_Drow)CUDA_CHECK(cudaFree(d_Drow)); CUDA_CHECK(cudaMalloc(&d_Drow,drowN*sizeof(float))); drow_cap=drowN; }
    const long dqN=(long)B*Hq*S*D; static float* d_dq_accum=nullptr; static long dq_cap=0;
    if(dqN>dq_cap){ if(d_dq_accum)CUDA_CHECK(cudaFree(d_dq_accum)); CUDA_CHECK(cudaMalloc(&d_dq_accum,dqN*sizeof(float))); dq_cap=dqN; }
    CUDA_CHECK(cudaMemset(d_dq_accum,0,dqN*sizeof(float)));
    const int dBlock=256; const long dGrid=(drowN+(dBlock/32)-1)/(dBlock/32);
    compute_drowsum_v22<<<(unsigned)dGrid,dBlock>>>(d_dO,d_O,d_Drow,drowN);
    constexpr dim3 BLOCK(256);   // V31 step1: 1 consumer wg + 1 producer wg
    dim3 GRID(B, Hkv, S / Bc);
    gqa_backward_v31_kv<Br,Bc,D><<<GRID, BLOCK>>>(tma_K_sw,tma_V_sw,tma_Q_sw,tma_dO_sw,d_Drow,d_LSE,d_dK,d_dV,d_dq_accum,B,Hq,Hkv,G,S,scale);
    const int convBlock=256; const int convGrid=(int)((dqN+convBlock-1)/convBlock);
    convert_dq_accum_to_bf16_v5<<<convGrid,convBlock>>>(d_dq_accum,d_dQ,dqN);
}
...
int main(){
    ...
    CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaDeviceSynchronize());
    check("── V31 Br=64 Bc=64 single-wg-per-tile step1 (full dv/dk) (Hopper SM_90) ──", Nq, Nkv, d_dQ, d_dK, d_dV);
    ...
    }
    {
        KernelStats s = benchmarkKernel(
            [&](){ launch_gqa_backward_v31<Br2,Bc2,D>(
                d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale); },
            100, 10, bwd_flops);
        displayStats("GQA bwd V31 Br=64, Bc=64  single-wg-per-tile step1 (full dv/dk)  (Hopper SM_90)", s);
    }
