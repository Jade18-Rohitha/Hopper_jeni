...
// ===== V4d = V44 + 3D DE-ALIAS swizzle TMA-reduce (single-pass conflict-free dQ store, cuDNN UTMAREDG-style) =====
template<int Br, int Bc, int D>
__global__ void __launch_bounds__(384, 1)
gqa_backward_v4d_kv(
    const __grid_constant__ CUtensorMap tma_K_sw,
    ...
    static_assert(Br == 64 && Bc == 64 && D == 128, "V4d requires Br=Bc=64, D=128");
    constexpr int CONS = 256;
    constexpr int PD   = 3;
    ...
    for (int it = 0; it < nIter; it++) {
        ...
        const int db = it & 1;                                        // V44: ping-pong dQ-stage buffer
        float* stageDQ = (wg == 0) ? sS[db] : sdP[db];
        store_acc_dealias(dq, stageDQ, wtid, scale);                  // V4d: de-aliased swizzled store (smem_row=a*8+bg)
        if (wg == 0) consumer_sync_wg0(); else consumer_sync_wg1();
        fence_proxy_async_shared();                 // generic STS staging -> async-proxy (TMA) read
        if (wtid == 0) {                            // swizzled TMA-reduce: 2 atoms (cols [wg*64,+32),[+32,+64)) -> dq_accum, off L1TEX
            const uint32_t bg = (uint32_t)(lBaseOf(gC, qcC) >> 3);   // V4d: 3D coord dim1 = row>>3 (row base is 64-aligned)
            tma_reduce_add_3d_v4d(&tma_dq_red, stageDQ,           (uint32_t)(wg * 64),      bg, 0u);   // atom 0
            tma_reduce_add_3d_v4d(&tma_dq_red, stageDQ + 64 * 32, (uint32_t)(wg * 64 + 32), bg, 0u);   // atom 1
            tma_store_commit_v34();
            tma_bulk_wait1_v43();                   // DOUBLE-BUFFER: keep <=1 reduce pending -> reduce(it) overlaps compute(it+1);
                                                    // reduce(it-1) is done here, so store(it+1) into buf[(it+1)&1]==buf[(it-1)&1] is safe.
        }
...
void launch_gqa_backward_v4d(
    ...
    // fp32 dq_accum swizzled TMA-reduce descriptor. SW128B needs box inner = 32 fp32 (=128B atom); the 64-wide
    // dQ D-half is reduced as TWO 32-wide atoms. Probe-confirmed: FP32 SW128B box{64,64} is rejected, box{32,64} OK.
    // V4d: 3D de-alias reduce descriptor. dims {col, bg(=row>>3), a(=row&7)}; box{32,8,8}; strides {8D, D}.
    // smem box layout addr = a*(8*32)+bg*32+col -> smem_row=a*8+bg (phase &7 = bg, de-aliases r0/r1);
    // global row = bg*8+a (natural). Probe-confirmed the decreasing-stride encode is accepted.
    auto make_tma_red = [&](const float* ptr, uint64_t rows) {
        CUtensorMap desc{};
        ...
        CUresult r=cuTensorMapEncodeTiled(&desc,CU_TENSOR_MAP_DATA_TYPE_FLOAT32,3,(void*)ptr,gSize,gStride,box,eStride,
            CU_TENSOR_MAP_INTERLEAVE_NONE,CU_TENSOR_MAP_SWIZZLE_128B,CU_TENSOR_MAP_L2_PROMOTION_L2_256B,CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE);
        if(r!=CUDA_SUCCESS){const char*e;cuGetErrorString(r,&e);fprintf(stderr,"tma_red v4d: %s\n",e);exit(1);} return desc; };
    ...
    constexpr dim3 BLOCK(384);
    dim3 GRID(B, Hkv, S / Bc);
    gqa_backward_v4d_kv<Br,Bc,D><<<GRID, BLOCK>>>(
        tma_K_sw, tma_V_sw, tma_Q_sw, tma_dO_sw, tma_dV_st, tma_dK_st, tma_dq_red,
        d_Drow, d_LSE, d_dK, d_dV, B, Hq, Hkv, G, S, scale);
...
int main(){
    ...
    launch_gqa_backward_v4d<Br2,Bc2,D>(d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale);
    CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaDeviceSynchronize());
    check("── V4d Br=64 Bc=64 3D de-alias swizzle TMA-reduce dQ (Hopper SM_90) ──", Nq, Nkv, d_dQ, d_dK, d_dV);
    ...
    {
        KernelStats s = benchmarkKernel(
            [&](){ launch_gqa_backward_v4d<Br2,Bc2,D>(
                d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale); },
            100, 10, bwd_flops);
        displayStats("GQA bwd V4d Br=64, Bc=64  3D de-alias swizzle TMA-reduce dQ  (Hopper SM_90)", s);
    }
    {
        KernelStats s = benchmarkKernel(
