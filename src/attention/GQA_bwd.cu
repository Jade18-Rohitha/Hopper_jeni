...
gqa_backward_vw1_kv(
    ...
    constexpr int CONS = 256;
    constexpr int PD   = 4;   // Vw1: deeper Q/dO pipeline (was 3) to cut long_scoreboard

    __shared__ __align__(128)  bf16 sK_sw[Bc * D];
    __shared__ __align__(128)  bf16 sV_sw[Bc * D];
    __shared__ __align__(128)  bf16 sQ_sw [PD][Br * D];
    __shared__ __align__(128)  bf16 sdO_sw[PD][Br * D];
    __shared__ __align__(1024) float sS [1][Br * 64];   // Vw1: SINGLE-buffered dQ (frees 32KB for PD=4)   // V44: DOUBLE-BUFFERED SWIZZLED dQ stage (wg0); each buf = 2 SW128B atoms of [Br*32]
    __shared__ __align__(1024) float sdP[1][Br * 64];   // Vw1: SINGLE-buffered dQ   // V44: DOUBLE-BUFFERED SWIZZLED dQ stage (wg1); atom a at [a*Br*32], SW128B chunk^ (row&7)
    __shared__ __align__(1024) bf16  sP [Br * 64];
    __shared__ __align__(1024) bf16  sDS[Br * 64];
    __shared__                 float sLSE[PD][Br];
    __shared__                 float sD  [PD][Br];
    ...
    for (int it = 0; it < nIter; it++) {
        ...
        if (wtid == 0) {                            // swizzled TMA-reduce: 2 atoms (cols [wg*64,+32),[+32,+64)) -> dq_accum, off L1TEX
            ...
            tma_reduce_add_2d_v43(&tma_dq_red, stageDQ + 64 * 32,   (uint32_t)(wg * 64 + 32), crow);   // atom 1
            tma_store_commit_v34();
            tma_bulk_wait0_v43();                   // Vw1 single-buffer: drain reduce before next store (serializes)
        }
        if (++qcC == nQTiles) { qcC = qc0; ++gC; }
    }
...
void launch_gqa_backward_vw1(
    ...
    constexpr dim3 BLOCK(384);
    dim3 GRID(B, Hkv, S / Bc);
    gqa_backward_vw1_kv<Br,Bc,D><<<GRID, BLOCK>>>(
        tma_K_sw, tma_V_sw, tma_Q_sw, tma_dO_sw, tma_dV_st, tma_dK_st, tma_dq_red,
        d_Drow, d_LSE, d_dK, d_dV, B, Hq, Hkv, G, S, scale);
...
int main(){
    ...
    CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaDeviceSynchronize());
    check("── Vw1 Br=64 Bc=64 PD=4 + single-buffer dQ (deeper pipe) (Hopper SM_90) ──", Nq, Nkv, d_dQ, d_dK, d_dV);
    ...
    }
    {
        KernelStats s = benchmarkKernel(
            [&](){ launch_gqa_backward_vw1<Br2,Bc2,D>(
                d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale); },
            100, 10, bwd_flops);
        displayStats("GQA bwd Vw1 Br=64, Bc=64  PD=4 + single-buffer dQ (deeper pipe)  (Hopper SM_90)", s);
    }
    {
        KernelStats s = benchmarkKernel(
