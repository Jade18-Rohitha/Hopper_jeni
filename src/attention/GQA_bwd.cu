...
// V33: P write ×4 via warp shuffle-repack. Each even-tig lane gathers its odd neighbour's
// 2 bf16 (XOR-1 shuffle) and emits ONE STS.64 for the 4-col block — half the store wavefronts
// (L1TEX) traded for a shuffle (ALU, which has headroom). Bit-identical: same P values, repacked.
template<int Bc>
__device__ __forceinline__ void fused_p_from_acc_v33(
    const float acc[32], bf16* sP, const float* sLSE, int wtid,
    int q_row0, int k_row0, float scale2)
{
    const int w = wtid >> 5, lane = wtid & 31;
    ...
    const int base0 = r0 * 64 + cc, m0 = r0 & 7;
    const int base1 = r1 * 64 + cc, m1 = r1 & 7;
    const bool wr = ((lane & 1) == 0);   // even-tig lane emits the STS.64
#pragma unroll
    for (int nt = 0; nt < Bc / 8; nt++) {
        const int c   = nt * 8 + cc;
        const int gc0 = k_row0 + c, gc1 = gc0 + 1;
        ...
        const float2 p1 = make_float2((gc0 > gr1) ? 0.f : ex2_approx_v29(__fmaf_rn(acc[nt * 4 + 2], scale2, -l1)),
                                      (gc1 > gr1) ? 0.f : ex2_approx_v29(__fmaf_rn(acc[nt * 4 + 3], scale2, -l1)));
        __nv_bfloat162 bp0 = __float22bfloat162_rn(p0), bp1 = __float22bfloat162_rn(p1);
        uint32_t q0 = *reinterpret_cast<uint32_t*>(&bp0), q1 = *reinterpret_cast<uint32_t*>(&bp1);
        uint32_t n0 = __shfl_xor_sync(0xffffffffu, q0, 1), n1 = __shfl_xor_sync(0xffffffffu, q1, 1);
        if (wr) {
            *reinterpret_cast<uint64_t*>(&sP[b0]) = (uint64_t)q0 | ((uint64_t)n0 << 32);
            *reinterpret_cast<uint64_t*>(&sP[b1]) = (uint64_t)q1 | ((uint64_t)n1 << 32);
        }
    }
}
template<int Bc>
__device__ __forceinline__ void fused_p_nomask_v33(
    const float acc[32], bf16* sP, const float* sLSE, int wtid, float scale2)
{
    const int w = wtid >> 5, lane = wtid & 31;
    const int r0 = w * 16 + (lane >> 2), r1 = r0 + 8, cc = (lane & 3) * 2;
...
// V33 = V32 + P write ×4 (warp shuffle-repack -> STS.64). Bit-identical.
template<int Br, int Bc, int D>
__global__ void __launch_bounds__(384, 1)
gqa_backward_v33_kv(
    const __grid_constant__ CUtensorMap tma_K_sw,
    ...
    for (int it = 0; it < nIter; it++) {
        ...
        if (wg == 0) {
            ...
            run_gemm_n64_sw2(acc, sQ_sw[s], sK_sw);   // V32: S-GEMM runs UNDER the sLSE LDG latency
            consumer_sync_wg0();                        // V32: sLSE RAW barrier here (after S-GEMM) — LSE load now hidden, not serial
            if (qcC == qc0) fused_p_from_acc_v33<Bc>(acc, sP, sLSE, wtid, q_row0, k_row0, scale2);
            else            fused_p_nomask_v33<Bc>(acc, sP, sLSE, wtid, scale2);
        } else {
            float acc[32]; zeroN<32>(acc);
            run_gemm_n64_sw2(acc, sdO_sw[s], sV_sw);
            store_acc_smem_v6<Bc, SS_STRIDE_V24>(acc, sdP, wtid, 1.0f);
...
int main(){
    ...
    CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaDeviceSynchronize());
    check("── V33 Br=64 Bc=64 P write x4 (shuffle-repack STS.64) (Hopper SM_90) ──", Nq, Nkv, d_dQ, d_dK, d_dV);
    ...
    }
    {
        KernelStats s = benchmarkKernel(
            [&](){ launch_gqa_backward_v33<Br2,Bc2,D>(
                d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale); },
            100, 10, bwd_flops);
        displayStats("GQA bwd V33 Br=64, Bc=64  P write x4 (shuffle-repack)  (Hopper SM_90)", s);
    }
