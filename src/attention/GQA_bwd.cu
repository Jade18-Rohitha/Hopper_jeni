...
// V31: derive sibling wgmma descriptors by a compile-time offset add (32-bit, no IMAD.X carry)
// instead of rebuilding cvta+shr+and per k-step — the forward's desc_add_lo.  Address field is
// bits[13:0]=(byte_addr>>4); a bf16 elem offset E (=2E bytes) shifts it by E>>3.  Safe: our smem
// (204,880 B) keeps address_field + max offset < 2^14, so no carry out of the field.
__device__ __forceinline__ uint64_t desc_add_lo_v31(uint64_t d, int elem_off) {
    uint32_t lo = (uint32_t)d + (uint32_t)(elem_off >> 3);
    return ((uint64_t)(uint32_t)(d >> 32) << 32) | (uint64_t)lo;
}
__device__ __forceinline__ void run_gemm_n64_sw2_d31(float acc[32], const bf16* A_sw, const bf16* B_sw) {
    fence_proxy_async_shared(); fence_operandN<32>(acc); wgmma_fence();
    const uint64_t dA0 = make_desc_sw128_K(A_sw), dB0 = make_desc_sw128_K(B_sw);
#pragma unroll
    for (int k = 0; k < 8; k++) {
        const int off = (k >> 2) * 4096 + (k & 3) * 16;
        wgmma_m64n64k16(acc, desc_add_lo_v31(dA0, off), desc_add_lo_v31(dB0, off));
    }
    wgmma_commit(); wgmma_wait0(); fence_operandN<32>(acc);
}
__device__ __forceinline__ void run_gemm_dVdK_half_te_issue_d31(float acc[32], const bf16* sP_sw, const bf16* B_sw_half) {
    fence_proxy_async_shared(); fence_operandN<32>(acc); wgmma_fence();
    const uint64_t dA0 = make_desc_sw128_MN(sP_sw), dB0 = make_desc_sw128_MN(B_sw_half);
#pragma unroll
    for (int k = 0; k < 4; k++)
        wgmma_m64n64k16_tAtB(acc, desc_add_lo_v31(dA0, k * 1024), desc_add_lo_v31(dB0, k * 1024));
    wgmma_commit();
}
__device__ __forceinline__ void run_gemm_dVdK_half_te_d31(float acc[32], const bf16* sP_sw, const bf16* B_sw_half) {
    fence_proxy_async_shared(); fence_operandN<32>(acc); wgmma_fence();
    const uint64_t dA0 = make_desc_sw128_MN(sP_sw), dB0 = make_desc_sw128_MN(B_sw_half);
#pragma unroll
    for (int k = 0; k < 4; k++)
...
__device__ __forceinline__ void run_gemm_dQ_half_te_d31(float acc[32], const bf16* sP_sw, const bf16* K_sw_atom) {
    fence_proxy_async_shared(); fence_operandN<32>(acc); wgmma_fence();
    const uint64_t dA0 = make_desc_sw128_K(sP_sw), dB0 = make_desc_sw128_MN(K_sw_atom);
#pragma unroll
    for (int k = 0; k < 4; k++)
        wgmma_m64n64k16_tB(acc, desc_add_lo_v31(dA0, k * 16), desc_add_lo_v31(dB0, k * 1024));
    wgmma_commit(); wgmma_wait0(); fence_operandN<32>(acc);
}
...
gqa_backward_v31_kv(
    ...
    for (int it = 0; it < nIter; it++) {
        ...
        if (wg == 0) {
            float acc[32]; zeroN<32>(acc);
            run_gemm_n64_sw2_d31(acc, sQ_sw[s], sK_sw);
            if (qcC == qc0) fused_p_from_acc_v29<Bc>(acc, sP, sLSE, wtid, q_row0, k_row0, scale2);
            else            fused_p_nomask_v29<Bc>(acc, sP, sLSE, wtid, scale2);
        } else {
            float acc[32]; zeroN<32>(acc);
            run_gemm_n64_sw2_d31(acc, sdO_sw[s], sV_sw);
            store_acc_smem_v6<Bc, SS_STRIDE_V24>(acc, sdP, wtid, 1.0f);
        }
        consumer_sync();   // wg0's swizzled sP writes visible cross-warp before the transposed reads

        // dV += Pᵀ·dO  — TRANSPOSE-ELIMINATED: A = Pᵀ read DIRECTLY from swizzled sP.
        run_gemm_dVdK_half_te_issue_d31(dv, sP, sdO_sw[s] + wg * 4096);   // V27: issue only; overlaps dS below
        ...
        // dK += dSᵀ·Q  — TRANSPOSE-ELIMINATED: A = dSᵀ read DIRECTLY from swizzled sP.
        run_gemm_dVdK_half_te_d31(dk, sDS, sQ_sw[s] + wg * 4096);
        consumer_sync();
        if (tid == 0) mbar_arrive_v11(&empty[s]);
        ...
        // (sP=dS already made cross-warp visible by the dK consumer_sync above).
        { float acc[32]; zeroN<32>(acc);
          run_gemm_dQ_half_te_d31(acc, sDS, sK_sw + wg * 4096);
          store_acc_smem_v6<64, SS_STRIDE_V24>(acc, (wg == 0) ? sS : sdP, wtid, scale); }
        consumer_sync();
        atomic_flush_stage_s<Br, 64, SS_STRIDE_V24>((wg == 0) ? sS : sdP, d_dq_accum, lBaseOf(gC, qcC) * D, D, wg * 64, wtid);
        consumer_sync();
...
int main(){
    ...
    CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaDeviceSynchronize());
    check("── V31 Br=64 Bc=64 desc_add_lo (sibling wgmma descriptors) (Hopper SM_90) ──", Nq, Nkv, d_dQ, d_dK, d_dV);
    ...
    }
    {
        KernelStats s = benchmarkKernel(
            [&](){ launch_gqa_backward_v31<Br2,Bc2,D>(
                d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale); },
            100, 10, bwd_flops);
        displayStats("GQA bwd V31 Br=64, Bc=64  desc_add_lo (sibling wgmma descriptors)  (Hopper SM_90)", s);
    }
