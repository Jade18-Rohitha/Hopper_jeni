...
gqa_backward_v47_kv(
    ...
    const int wg    = tid >> 7;
    const int wtid  = tid & 127;
    const int b = rasterize ? blockIdx.z : blockIdx.x;   // V47: adaptive raster (launcher sets flag by L2 footprint)
    const int hkv = blockIdx.y;
...
void launch_gqa_backward_v47(
    ...
    constexpr dim3 BLOCK(384);
    // V47: adaptive grid order. The L2 raster (k_tile fastest) helps ONLY when Q/dO thrashes L2
...
int main(){
    ...
    CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaDeviceSynchronize());
    check("── V47 Br=64 Bc=64 adaptive raster (default<=B4, raster B8) (Hopper SM_90) ──", Nq, Nkv, d_dQ, d_dK, d_dV);
    ...
    }
    {
        KernelStats s = benchmarkKernel(
            [&](){ launch_gqa_backward_v47<Br2,Bc2,D>(
                d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale); },
            100, 10, bwd_flops);
        displayStats("GQA bwd V47 Br=64, Bc=64  adaptive raster (default<=B4, raster B8)  (Hopper SM_90)", s);
    }
