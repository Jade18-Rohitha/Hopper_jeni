...
// ===== V5q = Q-PARALLEL FUSED: dQ one-writer (register-accum over kv, NO scatter store) + dK/dV TMA-reduce =====
// Brings all learnings together: V44's 27.6M-conflict dQ scatter store DOESN'T EXIST here. Each block owns a
// q-tile, streams its causal kv-tiles, accumulates dQ in registers across kv, writes dQ ONCE (one-writer TMA).
// dK/dV become the reduce axis (over q-tiles across blocks) -> swizzled fp32 stage + cp.reduce.async TMA-reduce
// into L2-resident dk/dv accum (cuDNN's mechanism: op_red, L2-hit, NOT atomics), then convert. Single-buffered
// stages (dQ freed its 64KB, spent on dK+dV stages). Grid Q-parallel: GRID(B, Hq, S/Br).
template<int Br, int Bc, int D>
__global__ void __launch_bounds__(384, 1)
gqa_backward_v5q(
    const __grid_constant__ CUtensorMap tma_Q_sw,
    ...
    const __grid_constant__ CUtensorMap tma_K_sw,
    const __grid_constant__ CUtensorMap tma_V_sw,
    const __grid_constant__ CUtensorMap tma_dQ_st,
    ...
    static_assert(Br == 64 && Bc == 64 && D == 128, "V5q requires Br=Bc=64, D=128");
    constexpr int PD = 3;
    ...
    __shared__ __align__(1024) bf16  sP  [Br * 64];
    __shared__ __align__(1024) bf16  sDS [Br * 64];
    __shared__ __align__(1024) float sStgK[2][Br * 64];   // swizzled fp32 dK stage (wg0/wg1), 2 SW128B atoms each
    __shared__ __align__(1024) float sStgV[2][Br * 64];   // swizzled fp32 dV stage
    __shared__                 float sLSE[Br];
    __shared__                 float sD  [Br];
    __shared__ __align__(8)    uint64_t mbar_qo;
    __shared__ __align__(8)    uint64_t d_ready;
    __shared__ __align__(8)    uint64_t full [PD];
    __shared__ __align__(8)    uint64_t empty[PD];
    ...
    if (tid == 0) {
        mbar_init_v4(&mbar_qo, 1); mbar_init_v4(&d_ready, 1);
        #pragma unroll
    ...
    if (wg == 2) {
        ...
        if (leader) {
            mbar_expect_tx_v4(&mbar_qo, bytesTile * 2);
        ...
        if (pl < Br) { sD[pl] = d_Drow[qbase + pl]; sLSE[pl] = d_LSE[qbase + pl]; }
        producer_sync();
        if (leader) mbar_arrive_v11(&d_ready);
        uint32_t epar[PD] = {0};
        for (int kc = 0; kc <= q_tile; kc++) {
            const int s = kc % PD;
            if (kc >= PD) { mbar_wait_v4(&empty[s], epar[s]); epar[s] ^= 1; }
            if (leader) {
                const uint32_t kr = (uint32_t)((b * Hkv + hkv) * S + kc * Bc);
                mbar_expect_tx_v4(&full[s], bytesAtom * 4);
                tma_load_2d_v4(&tma_K_sw, sK_sw[s],         &full[s], 0,  kr);
                tma_load_2d_v4(&tma_K_sw, sK_sw[s] + 64*64, &full[s], 64, kr);
                tma_load_2d_v4(&tma_V_sw, sV_sw[s],         &full[s], 0,  kr);
                tma_load_2d_v4(&tma_V_sw, sV_sw[s] + 64*64, &full[s], 64, kr);
            }
        }
        return;
    }
    ...
    const bf16* sdOhalf = sdO_sw + wg * 4096;
    const bf16* sQhalf  = sQ_sw  + wg * 4096;
    float* stgK = sStgK[wg];
    float* stgV = sStgV[wg];

    float dq[32]; zeroN<32>(dq);                 // PERSISTENT one-writer dQ accumulator
    uint32_t cpar[PD] = {0};
    for (int kc = 0; kc <= q_tile; kc++) {
        const int s = kc % PD;
        mbar_wait_v4(&full[s], cpar[s]); cpar[s] ^= 1;
        const uint64_t descGemmB = make_desc_sw128_K((wg == 0) ? sK_sw[s] : sV_sw[s]);
        const uint64_t descKhalf = make_desc_sw128_MN(sK_sw[s] + wg * 4096);

        float dPacc[32];
        if (wg == 0) {
            float acc[32]; zeroN<32>(acc);
            run_gemm_n64_sw2_hoB(acc, sQ_sw, descGemmB);
            if (kc == q_tile) fused_p_stsm<Bc, true >(acc, sP, sLSE, wtid, q_row0, kc * Bc, scale2);
            else              fused_p_stsm<Bc, false>(acc, sP, sLSE, wtid, 0, 0, scale2);
        } else {
            zeroN<32>(dPacc);
            run_gemm_n64_sw2_hoB(dPacc, sdO_sw, descGemmB);
        }
        consumer_sync();

        float dv[32]; zeroN<32>(dv);
        run_gemm_dVdK_half_te_issue_hoA(dv, descP, sdOhalf);
        if (wg == 1) fuse_dS_ldstsm<Bc>(sP, dPacc, sD, sDS, wtid);
        consumer_sync();

        float dk[32]; zeroN<32>(dk);
        run_gemm_dKdQ_te_issue_ho(dk, dq, descDSmn, descDSk, descKhalf, sQhalf);
        run_gemm_dVdKdQ_te_wait(dv, dk, dq);

        // dK/dV -> swizzled fp32 stage -> L2-resident TMA-reduce (cuDNN op_red mechanism)
        store_acc_sw128_f32(dk, stgK, wtid, scale);
        store_acc_sw128_f32(dv, stgV, wtid, 1.0f);
        if (wg == 0) consumer_sync_wg0(); else consumer_sync_wg1();
        fence_proxy_async_shared();
        if (wtid == 0) {
            const uint32_t krow = (uint32_t)((b * Hkv + hkv) * S + kc * Bc);
            tma_reduce_add_2d_v43(&tma_dk_red, stgK,           (uint32_t)(wg * 64),      krow);
            tma_reduce_add_2d_v43(&tma_dk_red, stgK + 64 * 32, (uint32_t)(wg * 64 + 32), krow);
            tma_reduce_add_2d_v43(&tma_dv_red, stgV,           (uint32_t)(wg * 64),      krow);
            tma_reduce_add_2d_v43(&tma_dv_red, stgV + 64 * 32, (uint32_t)(wg * 64 + 32), krow);
            tma_store_commit_v34();
            tma_bulk_wait0_v43();     // single-buffer: drain before next kc reuses the stage
        }
        consumer_sync();
        if (tid == 0) mbar_arrive_v11(&empty[s]);
    }

    // dQ one-writer epilogue: stage bf16 (per-wg D-half) -> single TMA-store to dQ[b,hq,q_tile]
    bf16* stageDQ = (wg == 0) ? sP : sDS;
    fence_operandN<32>(dq);
    stage_acc_bf16_s<64, 64>(dq, stageDQ, wtid, scale);
    consumer_sync();
    fence_proxy_async_shared();
    if (wtid == 0) {
        tma_store_2d_v34(&tma_dQ_st, stageDQ, (uint32_t)(wg * 64), qFlatRow);
        tma_store_commit_v34();
        tma_store_wait_v34();
    }
}
...
void launch_gqa_backward_v5q(
    ...
    static_assert(Br == 64 && Bc == 64 && D == 128, "V5q requires Br=Bc=64, D=128");
    auto make_tma_sw128 = [&](const bf16* ptr, uint64_t total_rows, uint32_t tile_rows) {
        CUtensorMap desc{}; uint64_t gSize[2]={(uint64_t)D,total_rows}; uint64_t gStride[1]={(uint64_t)D*sizeof(bf16)};
        uint32_t box[2]={64u,tile_rows}; uint32_t eStride[2]={1,1};
        CUresult r=cuTensorMapEncodeTiled(&desc,CU_TENSOR_MAP_DATA_TYPE_BFLOAT16,2,(void*)ptr,gSize,gStride,box,eStride,
            CU_TENSOR_MAP_INTERLEAVE_NONE,CU_TENSOR_MAP_SWIZZLE_128B,CU_TENSOR_MAP_L2_PROMOTION_L2_256B,CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE);
        if(r!=CUDA_SUCCESS){const char*e;cuGetErrorString(r,&e);fprintf(stderr,"v5q sw128: %s\n",e);exit(1);} return desc; };
    auto make_tma_out=[&](const bf16* ptr,uint64_t rows){
        CUtensorMap desc{}; uint64_t gSize[2]={(uint64_t)D,rows}; uint64_t gStride[1]={(uint64_t)D*sizeof(bf16)};
        uint32_t box[2]={64u,64u}; uint32_t eStride[2]={1,1};
        CUresult r=cuTensorMapEncodeTiled(&desc,CU_TENSOR_MAP_DATA_TYPE_BFLOAT16,2,(void*)ptr,gSize,gStride,box,eStride,
            CU_TENSOR_MAP_INTERLEAVE_NONE,CU_TENSOR_MAP_SWIZZLE_NONE,CU_TENSOR_MAP_L2_PROMOTION_L2_256B,CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE);
        if(r!=CUDA_SUCCESS){const char*e;cuGetErrorString(r,&e);fprintf(stderr,"v5q out: %s\n",e);exit(1);} return desc; };
    auto make_tma_red=[&](const float* ptr,uint64_t rows){
        CUtensorMap desc{}; uint64_t gSize[2]={(uint64_t)D,rows}; uint64_t gStride[1]={(uint64_t)D*sizeof(float)};
        uint32_t box[2]={32u,64u}; uint32_t eStride[2]={1,1};
        CUresult r=cuTensorMapEncodeTiled(&desc,CU_TENSOR_MAP_DATA_TYPE_FLOAT32,2,(void*)ptr,gSize,gStride,box,eStride,
            CU_TENSOR_MAP_INTERLEAVE_NONE,CU_TENSOR_MAP_SWIZZLE_128B,CU_TENSOR_MAP_L2_PROMOTION_L2_256B,CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE);
        if(r!=CUDA_SUCCESS){const char*e;cuGetErrorString(r,&e);fprintf(stderr,"v5q red: %s\n",e);exit(1);} return desc; };
    const uint64_t Rq=(uint64_t)B*Hq*S, Rkv=(uint64_t)B*Hkv*S;
    CUtensorMap tma_Q_sw =make_tma_sw128(d_Q, Rq, Br);
    CUtensorMap tma_dO_sw=make_tma_sw128(d_dO,Rq, Br);
    CUtensorMap tma_K_sw =make_tma_sw128(d_K, Rkv,Bc);
    CUtensorMap tma_V_sw =make_tma_sw128(d_V, Rkv,Bc);
    CUtensorMap tma_dQ_st=make_tma_out(d_dQ, Rq);

    const long drowN=(long)B*Hq*S;
    static float* d_Drow=nullptr; static long drow_cap=0;
    if(drowN>drow_cap){ if(d_Drow)CUDA_CHECK(cudaFree(d_Drow)); CUDA_CHECK(cudaMalloc(&d_Drow,drowN*sizeof(float))); drow_cap=drowN; }
    const long kvN=(long)B*Hkv*S*D;
    static float* d_dk_acc=nullptr; static float* d_dv_acc=nullptr; static long kv_cap=0;
    if(kvN>kv_cap){ if(d_dk_acc)CUDA_CHECK(cudaFree(d_dk_acc)); if(d_dv_acc)CUDA_CHECK(cudaFree(d_dv_acc));
        CUDA_CHECK(cudaMalloc(&d_dk_acc,kvN*sizeof(float))); CUDA_CHECK(cudaMalloc(&d_dv_acc,kvN*sizeof(float))); kv_cap=kvN; }
    CUDA_CHECK(cudaMemset(d_dk_acc,0,kvN*sizeof(float)));
    CUDA_CHECK(cudaMemset(d_dv_acc,0,kvN*sizeof(float)));
    CUtensorMap tma_dk_red=make_tma_red(d_dk_acc, Rkv);
    CUtensorMap tma_dv_red=make_tma_red(d_dv_acc, Rkv);
    ...
    constexpr dim3 BLOCK(384);
    dim3 GRID(B, Hq, S / Br);
    gqa_backward_v5q<Br,Bc,D><<<GRID,BLOCK>>>(
        tma_Q_sw, tma_dO_sw, tma_K_sw, tma_V_sw, tma_dQ_st, tma_dk_red, tma_dv_red,
        d_Drow, d_LSE, B, Hq, Hkv, G, S, scale);

    const int convBlock=256;
    convert_dq_accum_to_bf16_v5<<<(int)((kvN+convBlock-1)/convBlock),convBlock>>>(d_dk_acc, d_dK, kvN);
    convert_dq_accum_to_bf16_v5<<<(int)((kvN+convBlock-1)/convBlock),convBlock>>>(d_dv_acc, d_dV, kvN);
}
...
int main(){
    ...
    launch_gqa_backward_v5q<Br2,Bc2,D>(d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale);
    CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaDeviceSynchronize());
    check("── V5q Br=64 Bc=64 Q-PARALLEL: dQ one-writer + dK/dV TMA-reduce (Hopper SM_90) ──", Nq, Nkv, d_dQ, d_dK, d_dV);
    ...
    {
        KernelStats s = benchmarkKernel(
            [&](){ launch_gqa_backward_v5q<Br2,Bc2,D>(
                d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale); },
            100, 10, bwd_flops);
        displayStats("GQA bwd V5q Br=64, Bc=64  Q-PARALLEL dQ one-writer + dK/dV TMA-reduce  (Hopper SM_90)", s);
    }
    {
        KernelStats s = benchmarkKernel(
