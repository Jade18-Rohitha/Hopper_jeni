...
// V4c: cuDNN-style transpose-through-smem dQ store (decoded from cuDNN SASS: STS->LDS.64->STS.128->UTMAREDG).
// The mma-C fp32 fragment can't do a coalesced row-contiguous store directly (a thread owns a 2x2 block
// spanning rows r0 and r1=r0+8). Fix WITHOUT a CTA barrier: the transpose is INTRA-WARP — each warp owns
// 16 contiguous rows (w*16..w*16+15), so it writes its fragment to a per-warp swizzled scratch, __syncwarp,
// then reads its own 16 rows back as row-contiguous 4-fp32 chunks and writes them CONTIGUOUSLY (STS.128,
// conflict-free by construction) into the packed stage that the SWIZZLE_NONE box{64,64} TMA-reduce reads.
// Scratch is a FREE intermediate (no TMA constraint) -> a 4-fp32-chunk XOR swizzle (chunk^(row&7)) de-aliases
// the 8-row bank collapse -> ~2-way scratch store; the final contiguous STS.128 is 0-way. No padding needed.
// scr = this warpgroup's 4*1024 fp32 scratch region; stage = this wg's packed 64x64 fp32 dQ stage.
__device__ __forceinline__ void store_acc_transpose(const float* d, float* stage, float* scr, int wtid, float scl) {
    const int w = wtid >> 5, lane = wtid & 31;
    float* sw = scr   + w * (16 * 64);   // this warp's 16-row swizzled scratch
    float* st = stage + w * (16 * 64);   // this warp's 16-row packed stage region
    const int lr0 = lane >> 2, lr1 = lr0 + 8, cc = (lane & 3) * 2;   // LOCAL rows 0..15
#pragma unroll
    for (int nt = 0; nt < 8; nt++) {
        const int c = nt * 8 + cc, chunk = c >> 2, lo = c & 3;
        const int p0 = lr0 * 64 + ((chunk ^ (lr0 & 7)) << 2) + lo;
        const int p1 = lr1 * 64 + ((chunk ^ (lr1 & 7)) << 2) + lo;
        sw[p0] = d[nt*4+0]*scl; sw[p0+1] = d[nt*4+1]*scl;
        sw[p1] = d[nt*4+2]*scl; sw[p1+1] = d[nt*4+3]*scl;
    }
    __syncwarp();
#pragma unroll
    for (int rnd = 0; rnd < 8; rnd++) {
        const int f = rnd * 128 + lane * 4;        // 0..1023 within this warp's 16 rows (col multiple of 4)
        const int row = f >> 6, col = f & 63, chunk = col >> 2;
        const int ps = row * 64 + ((chunk ^ (row & 7)) << 2);   // 4-fp32 chunk base in swizzled scratch
        *reinterpret_cast<float4*>(&st[f]) = make_float4(sw[ps], sw[ps+1], sw[ps+2], sw[ps+3]);
    }
}
...
// ===== V4c = V43 + cuDNN-style transpose-through-smem dQ store (contiguous STS.128, __syncwarp) =====
template<int Br, int Bc, int D>
__global__ void __launch_bounds__(384, 1)
gqa_backward_v4c_kv(
    const __grid_constant__ CUtensorMap tma_K_sw,
    ...
    static_assert(Br == 64 && Bc == 64 && D == 128, "V4c requires Br=Bc=64, D=128");
    constexpr int CONS = 256;
    constexpr int PD   = 3;
    ...
    __shared__ __align__(128)  bf16 sQ_sw [PD][Br * D];
    __shared__ __align__(128)  bf16 sdO_sw[PD][Br * D];
    __shared__ __align__(128)  float sS [1][Br * 64];   // V4c: SINGLE-BUFFERED packed 64x64 fp32 dQ stage (wg0, TMA-reduce src)
    __shared__ __align__(128)  float sdP[1][Br * 64];   // V4c: SINGLE-BUFFERED packed 64x64 fp32 dQ stage (wg1, TMA-reduce src)
    __shared__ __align__(128)  float sScr[2][4 * 1024]; // V4c: per-warpgroup transpose scratch (4 warps x 16 rows x 64, swizzled)
    __shared__ __align__(1024) bf16  sP [Br * 64];
    __shared__ __align__(1024) bf16  sDS[Br * 64];
    __shared__                 float sLSE[PD][Br];
    __shared__                 float sD  [PD][Br];
    ...
    for (int it = 0; it < nIter; it++) {
        ...
        store_acc_transpose(dq, stageDQ, sScr[wg], wtid, scale);       // V4c: transpose-thru-smem -> contiguous STS.128
        if (wg == 0) consumer_sync_wg0(); else consumer_sync_wg1();
        fence_proxy_async_shared();                 // generic STS staging -> async-proxy (TMA) read
        if (wtid == 0) {                            // TMA-reduce add: staged packed fp32 tile -> dq_accum
            tma_reduce_add_2d_v43(&tma_dq_red, stageDQ, (uint32_t)(wg * 64), (uint32_t)lBaseOf(gC, qcC));
            tma_store_commit_v34();
            tma_bulk_wait0_v43();                   // SINGLE-BUFFER: drain reduce before next store overwrites the stage
        }
        consumer_sync();
        if (tid == 0) mbar_arrive_v11(&empty[s]);
        if (++qcC == nQTiles) { qcC = qc0; ++gC; }
...
void launch_gqa_backward_v4c(
    ...
    constexpr dim3 BLOCK(384);
    dim3 GRID(B, Hkv, S / Bc);
    gqa_backward_v4c_kv<Br,Bc,D><<<GRID, BLOCK>>>(
        tma_K_sw, tma_V_sw, tma_Q_sw, tma_dO_sw, tma_dV_st, tma_dK_st, tma_dq_red,
        d_Drow, d_LSE, d_dK, d_dV, B, Hq, Hkv, G, S, scale);
...
int main(){
    ...
    launch_gqa_backward_v4c<Br2,Bc2,D>(d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale);
    CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaDeviceSynchronize());
    check("── V4c Br=64 Bc=64 transpose-thru-smem STS.128 dQ (cuDNN-style) (Hopper SM_90) ──", Nq, Nkv, d_dQ, d_dK, d_dV);
    ...
    {
        KernelStats s = benchmarkKernel(
            [&](){ launch_gqa_backward_v4c<Br2,Bc2,D>(
                d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale); },
            100, 10, bwd_flops);
        displayStats("GQA bwd V4c Br=64, Bc=64  transpose-thru-smem STS.128 dQ (cuDNN-style)  (Hopper SM_90)", s);
    }
    {
        KernelStats s = benchmarkKernel(
