...
// V48: TWO independent n64 gemms in ONE wgmma group (16 wgmma, single commit/wait0) so acc1
//   overlaps acc0. Used for wg1 dOV(acc0=descGemmB=V) || redundant QK(acc1=descKgemm=K) -> hides
//   the redundant QK behind dOV latency so wg1 phase-1 ~= dOV+reshuffle (balanced with wg0).
__device__ __forceinline__ void run_gemm2_n64_sw2_hoB(
    float acc0[32], const bf16* A0_sw, uint64_t descB0_base,
    float acc1[32], const bf16* A1_sw, uint64_t descB1_base) {
    fence_proxy_async_shared();
    fence_operandN<32>(acc0); fence_operandN<32>(acc1);
    wgmma_fence();
#pragma unroll
    for (int k = 0; k < 8; k++) {
        uint64_t dA = make_desc_sw128_K(A0_sw + (k >> 2) * 4096 + (k & 3) * 16);
        uint64_t dB = descB0_base + (uint64_t)((k >> 2) * 512 + (k & 3) * 2);
        wgmma_m64n64k16(acc0, dA, dB);
    }
#pragma unroll
    for (int k = 0; k < 8; k++) {
        uint64_t dA = make_desc_sw128_K(A1_sw + (k >> 2) * 4096 + (k & 3) * 16);
        uint64_t dB = descB1_base + (uint64_t)((k >> 2) * 512 + (k & 3) * 2);
        wgmma_m64n64k16(acc1, dA, dB);
    }
    wgmma_commit();
    wgmma_wait0();
    fence_operandN<32>(acc0); fence_operandN<32>(acc1);
...
// V47: register-P dS fusion. wg1 computes its OWN P from acc2 (redundant QK gemm, same wgmma-C
//   layout as dP), so dS needs NO ldmatrix from sP -> decouples dS from wg0 sP publish (removes
//   barrier2) AND deletes the 4 LDSM/thread. bf16 round-trip on P matches the sP store->ldmatrix
//   path exactly -> dS BIT-IDENTICAL. Combines fused_p_stsm P-transform + fuse_dS dS + stmatrix.
template<int Bc, bool MASK>
__device__ __forceinline__ void fuse_pdS_reg(
    const float acc2[32], const float dP[32], const float* __restrict__ sLSE,
    const float* __restrict__ sD, bf16* __restrict__ sDS, int wtid,
    int q_row0, int k_row0, float scale2)
{
    const int w = wtid >> 5, lane = wtid & 31;
    const int r0 = w * 16 + (lane >> 2), r1 = r0 + 8, cc = (lane & 3) * 2;
...
#pragma unroll
    for (int nt = 0; nt < 8; nt++) {
        float e00 = ex2_approx_v29(__fmaf_rn(acc2[nt*4+0], scale2, -l0));
        float e01 = ex2_approx_v29(__fmaf_rn(acc2[nt*4+1], scale2, -l0));
        float e10 = ex2_approx_v29(__fmaf_rn(acc2[nt*4+2], scale2, -l1));
        float e11 = ex2_approx_v29(__fmaf_rn(acc2[nt*4+3], scale2, -l1));
        if (MASK) {
            const int gc0 = k_row0 + nt*8 + cc, gc1 = gc0 + 1;
            e00 = (gc0 > gr0) ? 0.f : e00;  e01 = (gc1 > gr0) ? 0.f : e01;
            e10 = (gc0 > gr1) ? 0.f : e10;  e11 = (gc1 > gr1) ? 0.f : e11;
        }
        const float2 pf0 = __bfloat1622float2(__float22bfloat162_rn(make_float2(e00, e01)));  // bf16 round-trip
        const float2 pf1 = __bfloat1622float2(__float22bfloat162_rn(make_float2(e10, e11)));
        const float d00 = pf0.x * (dP[nt*4+0] - D0), d01 = pf0.y * (dP[nt*4+1] - D0);
        const float d10 = pf1.x * (dP[nt*4+2] - D1), d11 = pf1.y * (dP[nt*4+3] - D1);
        *reinterpret_cast<__nv_bfloat162*>(&pr0[nt]) = __float22bfloat162_rn(make_float2(d00, d01));
        *reinterpret_cast<__nv_bfloat162*>(&pr1[nt]) = __float22bfloat162_rn(make_float2(d10, d11));
...
gqa_backward_v45_kv(
    ...
    const int wg    = tid >> 7;
    const int wtid  = tid & 127;
    const int b = blockIdx.z, hkv = blockIdx.y, k_tile = blockIdx.x;  // V45: k_tile fastest (L2 locality)
    const int k_row0 = k_tile * Bc;
    const int nQTiles = S / Br;
...
void launch_gqa_backward_v45(
    ...
    constexpr dim3 BLOCK(384);
    dim3 GRID(S / Bc, Hkv, B);  // V45: k_tile on .x (fastest-varying)
...
gqa_backward_v46_kv(
    ...
    const int wg    = tid >> 7;
    const int wtid  = tid & 127;
    const int b = blockIdx.z, hkv = blockIdx.y, k_tile = blockIdx.x;  // V46: k_tile fastest (L2 locality)
    const int k_row0 = k_tile * Bc;
    const int nQTiles = S / Br;
...
void launch_gqa_backward_v46(
    ...
    constexpr dim3 BLOCK(384);
    dim3 GRID(S / Bc, Hkv, B);  // V46: k_tile on .x (fastest-varying)
    gqa_backward_v46_kv<Br,Bc,D><<<GRID, BLOCK>>>(
        tma_K_sw, tma_V_sw, tma_Q_sw, tma_dO_sw, tma_dV_st, tma_dK_st, tma_dq_red,
        d_Drow, d_LSE, d_dK, d_dV, B, Hq, Hkv, G, S, scale);
...
gqa_backward_v47_kv(
    ...
    __shared__ __align__(128)  bf16 sQ_sw [PD][Br * D];
    __shared__ __align__(128)  bf16 sdO_sw[PD][Br * D];
    __shared__ __align__(1024) float sS [2][Br * 64];   // V47: DOUBLE-BUFFERED SWIZZLED dQ stage (wg0); each buf = 2 SW128B atoms of [Br*32]
    __shared__ __align__(1024) float sdP[2][Br * 64];   // V47: DOUBLE-BUFFERED SWIZZLED dQ stage (wg1); atom a at [a*Br*32], SW128B chunk^ (row&7)
    __shared__ __align__(1024) bf16  sP [Br * 64];
    __shared__ __align__(1024) bf16  sDS[Br * 64];
    __shared__                 float sLSE[PD][Br];
    __shared__                 float sD  [PD][Br];
    ...
    const int wg    = tid >> 7;
    const int wtid  = tid & 127;
    const int b = blockIdx.z, hkv = blockIdx.y, k_tile = blockIdx.x;  // V47: k_tile fastest (L2 locality)
    const int k_row0 = k_tile * Bc;
    const int nQTiles = S / Br;
    ...
    for (int it = 0; it < nIter; it++) {
        ...
        consumer_sync();                            // V47: ONE barrier publishes BOTH sP and sDS (merged barrier1+barrier2)
        ...
        run_gemm_dKdQ_te_issue_ho(dk, dq, descDSmn, descDSk, descKhalf, sQ_sw[s] + wg * 4096);
        run_gemm_dVdKdQ_te_wait(dv, dk, dq);
        const int db = it & 1;                                        // V47: ping-pong dQ-stage buffer
        float* stageDQ = (wg == 0) ? sS[db] : sdP[db];
        store_acc_sw128_f32(dq, stageDQ, wtid, scale);               // CONFLICT-FREE swizzled store -> buf[it&1] (2 SW128B atoms)
        fence_proxy_async_shared();                 // generic STS staging -> async-proxy (TMA) read
        consumer_sync();                            // V47 FUSE: the end-of-iter FULL barrier now also orders
                                                    //   dq-store -> TMA-reduce (all 256 stores done). Deletes the
                                                    //   separate 128-thread split store->reduce barrier (4 -> 3 loop barriers).
        if (wtid == 0) {                            // swizzled TMA-reduce: 2 atoms -> dq_accum, off L1TEX
            const uint32_t crow = (uint32_t)lBaseOf(gC, qcC);
...
void launch_gqa_backward_v47(
    ...
    auto make_tma_red = [&](const float* ptr, uint64_t rows) {
        ...
        CUresult r=cuTensorMapEncodeTiled(&desc,CU_TENSOR_MAP_DATA_TYPE_FLOAT32,2,(void*)ptr,gSize,gStride,box,eStride,
            CU_TENSOR_MAP_INTERLEAVE_NONE,CU_TENSOR_MAP_SWIZZLE_128B,CU_TENSOR_MAP_L2_PROMOTION_L2_256B,CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE);
        if(r!=CUDA_SUCCESS){const char*e;cuGetErrorString(r,&e);fprintf(stderr,"tma_red v47: %s\n",e);exit(1);} return desc; };
    ...
    constexpr dim3 BLOCK(384);
    dim3 GRID(S / Bc, Hkv, B);  // V47: k_tile on .x (fastest-varying)
    gqa_backward_v47_kv<Br,Bc,D><<<GRID, BLOCK>>>(
        tma_K_sw, tma_V_sw, tma_Q_sw, tma_dO_sw, tma_dV_st, tma_dK_st, tma_dq_red,
        d_Drow, d_LSE, d_dK, d_dV, B, Hq, Hkv, G, S, scale);
...
gqa_backward_v48_kv(
    ...
    static_assert(Br == 64 && Bc == 64 && D == 128, "V48 requires Br=Bc=64, D=128");
    constexpr int CONS = 256;
    constexpr int PD   = 3;
    ...
    __shared__ __align__(128)  bf16 sQ_sw [PD][Br * D];
    __shared__ __align__(128)  bf16 sdO_sw[PD][Br * D];
    __shared__ __align__(1024) float sS [2][Br * 64];   // V48: DOUBLE-BUFFERED SWIZZLED dQ stage (wg0); each buf = 2 SW128B atoms of [Br*32]
    __shared__ __align__(1024) float sdP[2][Br * 64];   // V48: DOUBLE-BUFFERED SWIZZLED dQ stage (wg1); atom a at [a*Br*32], SW128B chunk^ (row&7)
    __shared__ __align__(1024) bf16  sP [Br * 64];
    __shared__ __align__(1024) bf16  sDS[Br * 64];
    __shared__                 float sLSE[PD][Br];
    __shared__                 float sD  [PD][Br];
    ...
    const int wg    = tid >> 7;
    const int wtid  = tid & 127;
    const int b = blockIdx.z, hkv = blockIdx.y, k_tile = blockIdx.x;  // V48: k_tile fastest (L2 locality)
    const int k_row0 = k_tile * Bc;
    const int nQTiles = S / Br;
    ...
    // V42: hoist the invariant-buffer wgmma base descriptors — computed once, reused every kv-tile.
    const uint64_t descGemmB = make_desc_sw128_K((wg == 0) ? sK_sw : sV_sw);   // S/dP GEMM B (K-major)
    const uint64_t descKgemm = make_desc_sw128_K(sK_sw);                        // V48: K desc for wg1 redundant QK (descGemmB is V on wg1)
    const uint64_t descP     = make_desc_sw128_MN(sP);                          // dV A (Major::MN)
    const uint64_t descDSmn  = make_desc_sw128_MN(sDS);                         // dK A (Major::MN)
    const uint64_t descDSk   = make_desc_sw128_K (sDS);                         // dQ A (Major::K)
    const uint64_t descKhalf = make_desc_sw128_MN(sK_sw + wg * 4096);           // dQ B (Major::MN)
    ...
    for (int it = 0; it < nIter; it++) {
        ...
        if (wg == 0) {
            ...
            zeroN<32>(dPacc);
            float acc2[32]; zeroN<32>(acc2);                             // V48: dOV || redundant-QK OVERLAPPED
            run_gemm2_n64_sw2_hoB(dPacc, sdO_sw[s], descGemmB, acc2, sQ_sw[s], descKgemm);  // one wgmma burst, one wait
            if (qcC == qc0) fuse_pdS_reg<Bc, true >(acc2, dPacc, sLSE[s], sD[s], sDS, wtid, q_row0, k_row0, scale2);
            else            fuse_pdS_reg<Bc, false>(acc2, dPacc, sLSE[s], sD[s], sDS, wtid, 0,       0,      scale2);
        }
        consumer_sync();                            // V48: ONE barrier publishes BOTH sP and sDS (merged barrier1+barrier2)
        ...
        run_gemm_dKdQ_te_issue_ho(dk, dq, descDSmn, descDSk, descKhalf, sQ_sw[s] + wg * 4096);
        run_gemm_dVdKdQ_te_wait(dv, dk, dq);
        const int db = it & 1;                                        // V48: ping-pong dQ-stage buffer
        float* stageDQ = (wg == 0) ? sS[db] : sdP[db];
        store_acc_sw128_f32(dq, stageDQ, wtid, scale);               // CONFLICT-FREE swizzled store -> buf[it&1] (2 SW128B atoms)
        fence_proxy_async_shared();                 // generic STS staging -> async-proxy (TMA) read
        consumer_sync();                            // V48 FUSE: the end-of-iter FULL barrier now also orders
                                                    //   dq-store -> TMA-reduce (all 256 stores done). Deletes the
                                                    //   separate 128-thread split store->reduce barrier (4 -> 3 loop barriers).
        if (wtid == 0) {                            // swizzled TMA-reduce: 2 atoms -> dq_accum, off L1TEX
            const uint32_t crow = (uint32_t)lBaseOf(gC, qcC);
...
void launch_gqa_backward_v48(
    ...
    auto make_tma_red = [&](const float* ptr, uint64_t rows) {
        ...
        CUresult r=cuTensorMapEncodeTiled(&desc,CU_TENSOR_MAP_DATA_TYPE_FLOAT32,2,(void*)ptr,gSize,gStride,box,eStride,
            CU_TENSOR_MAP_INTERLEAVE_NONE,CU_TENSOR_MAP_SWIZZLE_128B,CU_TENSOR_MAP_L2_PROMOTION_L2_256B,CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE);
        if(r!=CUDA_SUCCESS){const char*e;cuGetErrorString(r,&e);fprintf(stderr,"tma_red v48: %s\n",e);exit(1);} return desc; };
    ...
    constexpr dim3 BLOCK(384);
    dim3 GRID(S / Bc, Hkv, B);  // V48: k_tile on .x (fastest-varying)
    gqa_backward_v48_kv<Br,Bc,D><<<GRID, BLOCK>>>(
        tma_K_sw, tma_V_sw, tma_Q_sw, tma_dO_sw, tma_dV_st, tma_dK_st, tma_dq_red,
        d_Drow, d_LSE, d_dK, d_dV, B, Hq, Hkv, G, S, scale);
...
int main(){
    ...
    CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaDeviceSynchronize());
    check("── V45 Br=64 Bc=64 vec-store + L2 raster (Hopper SM_90) ──", Nq, Nkv, d_dQ, d_dK, d_dV);
    ...
    CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaDeviceSynchronize());
    check("── V47 Br=64 Bc=64 register-P dS (barrier1+2 merged, 3->2) (Hopper SM_90) ──", Nq, Nkv, d_dQ, d_dK, d_dV);

    launch_gqa_backward_v48<Br2,Bc2,D>(d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale);
    CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaDeviceSynchronize());
    check("── V48 Br=64 Bc=64 register-P dS + dOV||QK overlap (Hopper SM_90) ──", Nq, Nkv, d_dQ, d_dK, d_dV);
    ...
    }
    {
        KernelStats s = benchmarkKernel(
            [&](){ launch_gqa_backward_v45<Br2,Bc2,D>(
                d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale); },
            100, 10, bwd_flops);
        displayStats("GQA bwd V45 Br=64, Bc=64  vec-store + L2 raster  (Hopper SM_90)", s);
    }
    {
        KernelStats s = benchmarkKernel(
    ...
    }
    {
        KernelStats s = benchmarkKernel(
            [&](){ launch_gqa_backward_v47<Br2,Bc2,D>(
                d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale); },
            100, 10, bwd_flops);
        displayStats("GQA bwd V47 Br=64, Bc=64  register-P dS (barrier1+2 merged, 3->2)  (Hopper SM_90)", s);
    }
    {
        KernelStats s = benchmarkKernel(
            [&](){ launch_gqa_backward_v48<Br2,Bc2,D>(
                d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale); },
            100, 10, bwd_flops);
        displayStats("GQA bwd V48 Br=64, Bc=64  register-P dS + dOV||QK overlap  (Hopper SM_90)", s);
    }
