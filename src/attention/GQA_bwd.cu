...
// ===== Vw2 = V44 + UNIFORM registers (drop setmaxnreg) — register test on V44 base =====
template<int Br, int Bc, int D>
__global__ void __launch_bounds__(384, 1)
gqa_backward_vw2_kv(
    const __grid_constant__ CUtensorMap tma_K_sw,
    ...
    static_assert(Br == 64 && Bc == 64 && D == 128, "Vw2 requires Br=Bc=64, D=128");
    constexpr int CONS = 256;
    constexpr int PD   = 3;
    ...
    if (wg == 2) {
        /* Vw2: no setmaxnreg (uniform regs, like cuDNN) */
        const bool leader = (tid == 256);
        const int  pl     = tid - 256;
        if (leader) {
            mbar_expect_tx_v4(&mbar_kv, bytesAtom * 4);
    ...
    /* Vw2: no setmaxnreg (uniform regs) */
    const float scale2 = scale * LOG2E_V29;
    mbar_wait_v4(&mbar_kv, 0);
...
void launch_gqa_backward_vw2(
    ...
    constexpr dim3 BLOCK(384);
    dim3 GRID(B, Hkv, S / Bc);
    gqa_backward_vw2_kv<Br,Bc,D><<<GRID, BLOCK>>>(
        tma_K_sw, tma_V_sw, tma_Q_sw, tma_dO_sw, tma_dV_st, tma_dK_st, tma_dq_red,
        d_Drow, d_LSE, d_dK, d_dV, B, Hq, Hkv, G, S, scale);
...
int main(){
    ...
    launch_gqa_backward_vw2<Br2,Bc2,D>(d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale);
    CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaDeviceSynchronize());
    check("── Vw2 Br=64 Bc=64 V44 + UNIFORM regs (no setmaxnreg) (Hopper SM_90) ──", Nq, Nkv, d_dQ, d_dK, d_dV);
    ...
    {
        KernelStats s = benchmarkKernel(
            [&](){ launch_gqa_backward_vw2<Br2,Bc2,D>(
                d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale); },
            100, 10, bwd_flops);
        displayStats("GQA bwd Vw2 Br=64, Bc=64  V44 + UNIFORM regs (no setmaxnreg)  (Hopper SM_90)", s);
    }
    {
        KernelStats s = benchmarkKernel(
