...
// V42: DIRECT-atomic dQ flush — atomicAdd the dQ wgmma-C fragment straight to global, NO shared
// round-trip (deletes store_acc_smem_v6 + atomic_flush_stage_s + the wg-scoped barrier between them).
// Same (r,c)→value map as store_acc_smem_v6, same scale → bit-identical VALUES (modulo atomic order,
// already present in V41).  Trades the round-trip's shared LSU traffic (the 63% wall) for uncoalesced
// global REDG (L2, ~22% headroom).  Frees sS/sdP smem (36 KB).
template<int NCOL>
__device__ __forceinline__ void atomic_dq_reg(const float* d, float* g, long base, int D, int coloff, int tid, float scl) {
    const int w = tid >> 5, lane = tid & 31;
    const int r0 = w * 16 + (lane >> 2), r1 = r0 + 8, cc = (lane & 3) * 2;
#pragma unroll
    for (int nt = 0; nt < NCOL / 8; nt++) {
        const int c = nt * 8 + cc;
        atomicAdd(&g[base + (long)r0 * D + coloff + c + 0], d[nt * 4 + 0] * scl);
        atomicAdd(&g[base + (long)r0 * D + coloff + c + 1], d[nt * 4 + 1] * scl);
        atomicAdd(&g[base + (long)r1 * D + coloff + c + 0], d[nt * 4 + 2] * scl);
        atomicAdd(&g[base + (long)r1 * D + coloff + c + 1], d[nt * 4 + 3] * scl);
    }
}
...
// V42 = V41 + DIRECT-atomic dQ flush.  Kills the dQ store→read-back round-trip (the #1 shared-load
// source, ~76%): store_acc_smem_v6 + the wg-scoped barrier + atomic_flush_stage_s are replaced by a
// single atomic_dq_reg that atomicAdds the dQ wgmma-C fragment straight to d_dq_accum (uncoalesced,
// but no shared traffic).  sS/sdP DELETED (−36 KB smem).  Bit-identical modulo atomic ordering (V41
// already atomics).  Tests whether relieving the 63% LSU wall beats the added L2 atomic traffic.
template<int Br, int Bc, int D>
__global__ void __launch_bounds__(384, 1)
gqa_backward_v42_kv(
    const __grid_constant__ CUtensorMap tma_K_sw,
    ...
    __shared__ __align__(128)  bf16 sQ_sw [PD][Br * D];
    __shared__ __align__(128)  bf16 sdO_sw[PD][Br * D];
    // V42: sS/sdP DELETED — the dQ round-trip that used them is gone (direct-atomic flush).
    __shared__ __align__(1024) bf16  sP [Br * 64];
    __shared__ __align__(1024) bf16  sDS[Br * 64];
    __shared__                 float sLSE[PD][Br];
    __shared__                 float sD  [PD][Br];
    ...
    for (int it = 0; it < nIter; it++) {
        ...
        run_gemm_dKdQ_te_issue(dk, dq, sDS, sQ_sw[s] + wg * 4096, sK_sw + wg * 4096);
        run_gemm_dVdKdQ_te_wait(dv, dk, dq);
        // V42: direct-atomic dQ flush — NO shared round-trip (store_acc + wg-barrier + atomic_flush gone).
        atomic_dq_reg<64>(dq, d_dq_accum, lBaseOf(gC, qcC) * D, D, wg * 64, wtid, scale);
        consumer_sync();   // 256: BOTH wgs past dK's sQ_sw read before empty[s] frees sQ_sw for the producer
        if (tid == 0) mbar_arrive_v11(&empty[s]);
        if (++qcC == nQTiles) { qcC = qc0; ++gC; }
    }
...
int main(){
    ...
    CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaDeviceSynchronize());
    check("-- V42 Br=64 Bc=64 direct-atomic dQ flush (Hopper SM_90) --", Nq, Nkv, d_dQ, d_dK, d_dV);
    ...
    }
    {
        KernelStats s = benchmarkKernel(
            [&](){ launch_gqa_backward_v42<Br2,Bc2,D>(
                d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale); },
            100, 10, bwd_flops);
        displayStats("GQA bwd V42 Br=64, Bc=64  direct-atomic dQ flush  (Hopper SM_90)", s);
    }
