...
// V42: bf16 dQ round-trip stage.  Identical (r,c) map to store_acc_smem_v6, but the fp32 dQ partial
// (dq*scl) is rounded to bf16 ONCE here (packed bf162 STS.32) → HALVES the shared bytes of the dQ
// round-trip (the 76%-of-shared-LDS wall).  The global accumulator stays fp32 (atomic_flush_bf16
// widens back to fp32 before atomicAdd), so only this tile's partial is bf16-rounded, not the sum.
template<int NCOL, int ROWSTRIDE>
__device__ __forceinline__ void store_acc_smem_bf16(const float *d, bf16 *smem, int tid, float scl) {
    int w = tid >> 5, lane = tid & 31;
    int r0 = w * 16 + (lane >> 2), r1 = r0 + 8, cc = (lane & 3) * 2;
#pragma unroll
    for (int nt = 0; nt < NCOL / 8; nt++) {
        int c = nt * 8 + cc;
        *reinterpret_cast<__nv_bfloat162*>(&smem[r0 * ROWSTRIDE + c]) =
            __float22bfloat162_rn(make_float2(d[nt * 4 + 0] * scl, d[nt * 4 + 1] * scl));
        *reinterpret_cast<__nv_bfloat162*>(&smem[r1 * ROWSTRIDE + c]) =
            __float22bfloat162_rn(make_float2(d[nt * 4 + 2] * scl, d[nt * 4 + 3] * scl));
    }
}
...
// V42: bf16 read-back of the dQ round-trip.  Reads the bf16 stage (HALF the bytes of the fp32 path
// — this is the ~76%-of-shared-LDS bucket), widens to fp32, atomicAdds to the fp32 global accumulator
// (unchanged, coalesced).  Same coalesced consecutive-thread→consecutive-global-col mapping.
template<int ROWS, int NCOL, int ROWSTRIDE>
__device__ __forceinline__ void atomic_flush_bf16(const bf16 *stage, float *g, long base, int D, int coloff, int tid) {
    constexpr int N = ROWS * NCOL;
    for (int li = tid; li < N; li += 128) {
        int row = li / NCOL, col = li % NCOL;
        atomicAdd(&g[base + (long)row * D + coloff + col], __bfloat162float(stage[row * ROWSTRIDE + col]));
    }
}
...
// V42 = V41 + bf16 dQ round-trip.  sS/sdP become bf16 (−18 KB smem); store_acc_smem_bf16 rounds the
// fp32 dQ partial to bf16 ONCE, atomic_flush_bf16 widens back to fp32 before the (unchanged, coalesced)
// atomicAdd to the fp32 global accumulator.  HALVES the shared bytes of the dQ round-trip (the
// 76%-of-shared-LDS wall).  PRECISION-GATED: only this tile's partial is bf16-rounded; dK/dV exact.
template<int Br, int Bc, int D>
__global__ void __launch_bounds__(384, 1)
gqa_backward_v42_kv(
    const __grid_constant__ CUtensorMap tma_K_sw,
    ...
    __shared__ __align__(128)  bf16 sQ_sw [PD][Br * D];
    __shared__ __align__(128)  bf16 sdO_sw[PD][Br * D];
    __shared__ __align__(16)   bf16  sS [Br * SS_STRIDE_V24];   // V42: bf16 dQ stage (was fp32)
    __shared__ __align__(16)   bf16  sdP[Br * SS_STRIDE_V24];   // V42: bf16 dQ stage (was fp32)
    __shared__ __align__(1024) bf16  sP [Br * 64];
    __shared__ __align__(1024) bf16  sDS[Br * 64];
    __shared__                 float sLSE[PD][Br];
    __shared__                 float sD  [PD][Br];
    ...
    for (int it = 0; it < nIter; it++) {
        ...
        run_gemm_dKdQ_te_issue(dk, dq, sDS, sQ_sw[s] + wg * 4096, sK_sw + wg * 4096);
        run_gemm_dVdKdQ_te_wait(dv, dk, dq);
        // V42: bf16 dQ round-trip — half the shared bytes vs the fp32 store/read-back.
        store_acc_smem_bf16<64, SS_STRIDE_V24>(dq, (wg == 0) ? sS : sdP, wtid, scale);
        if (wg == 0) consumer_sync_wg0(); else consumer_sync_wg1();
        atomic_flush_bf16<Br, 64, SS_STRIDE_V24>((wg == 0) ? sS : sdP, d_dq_accum, lBaseOf(gC, qcC) * D, D, wg * 64, wtid);
        consumer_sync();
        if (tid == 0) mbar_arrive_v11(&empty[s]);
        if (++qcC == nQTiles) { qcC = qc0; ++gC; }
    }
...
int main(){
    ...
    CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaDeviceSynchronize());
    check("── V42 Br=64 Bc=64 bf16-dQ-round-trip (Hopper SM_90) ──", Nq, Nkv, d_dQ, d_dK, d_dV);
    ...
    }
    {
        KernelStats s = benchmarkKernel(
            [&](){ launch_gqa_backward_v42<Br2,Bc2,D>(
                d_Q,d_K,d_V,d_O,d_dO,d_LSE,d_dQ,d_dK,d_dV,B,Hq,Hkv,G,S,scale); },
            100, 10, bwd_flops);
        displayStats("GQA bwd V42 Br=64, Bc=64  bf16-dQ-round-trip  (Hopper SM_90)", s);
    }
